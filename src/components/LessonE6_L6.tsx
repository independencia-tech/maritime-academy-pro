...
// ══════════════════════════════════════
  export const BANK = {
    fr:[
      {q:"Qu'est-ce que l'ullage dans le contexte du jaugeage cargo ?",opts:["Le volume total de la citerne","La distance entre la surface du liquide et le haut de la citerne","Le poids du cargo","La temperature du cargo"],correct:1,expl:"L'ullage (ou 'creux' en francais) est la distance entre la surface du liquide et le point de reference en haut de la citerne (dome ou trou de jauge). Formule : Niveau = Hauteur totale citerne - Ullage. Plus l'ullage est grand, moins la citerne est remplie."},
      {q:"Quelle est la temperature de reference internationale pour le calcul du volume net ?",opts:["0°C","15°C","20°C","60°F"],correct:1,expl:"15°C (ou 60°F pour les pays anglosaxons utilisant ASTM) est la temperature de reference internationale selon ASTM D1250. Tous les volumes cargo sont calcules et declares a cette temperature pour permettre la comparaison entre differents ports et climats."},
      {q:"Le VCF (Volume Correction Factor) corrige le volume pour tenir compte de :",opts:["La pression atmospherique","La temperature du produit","La salinite de l'eau","La viscosite du cargo"],correct:1,expl:"Le VCF corrige le volume observe (a la temperature de mesure) en volume reference (a 15°C). VCF = f(temperature, densite/API). Si T > 15°C, le produit est dilate donc VCF < 1 (volume net < volume brut). Si T < 15°C, VCF > 1."},
      {q:"Quelle est la formule de base du calcul cargo ?",opts:["Masse = Volume x Pression","Masse = Volume net x Densite a 15°C","Masse = Ullage x Hauteur","Masse = VCF x Temperature"],correct:1,expl:"Masse (MT) = Volume net (m3 a 15°C) x Densite a 15°C (t/m3). Le volume net est obtenu en appliquant le VCF au volume brut. La densite est mesuree a 15°C ou corrigee via les tables ASTM. Cette formule est la base du Bill of Lading."},
      {q:"Quel type de jauge electonique offre la meilleure precision legale selon ASTM ?",opts:["Jauge a flotteur","Jauge radar (GTL)","Jauge servo-moteur","Transducteur ultrasonique"],correct:2,expl:"La jauge servo-moteur (+/- 0.5mm) est le standard de precision legale selon ASTM D3900. Elle utilise un disque discoide immerse dans le produit et mesure la force exercee. Elle peut aussi mesurer la densite. C'est le type de jauge de reference pour les operations de quantification legales."},
      {q:"Qu'est-ce qu'un Ullage Report ?",opts:["Le journal de bord du navire","Document signé par Chief Officer et inspecteur cargo independant","Plan de chargement","Rapport d'inspection PSC"],correct:1,expl:"L'Ullage Report est le document officiel enregistrant toutes les mesures de jaugeage : ullage, temperature, densite par citerne, calculs VCF, volumes bruts et nets, masse totale. Il est signe par le Chief Officer et l'inspecteur cargo independant (surveyor). C'est la base du Bill of Lading."},
      {q:"Quelle correction est appliquee pour tenir compte de l'assiette du navire lors du jaugeage ?",opts:["Correction de densite","Trim correction (correction d'assiette)","Correction de gite","Correction barometrique"],correct:1,expl:"La trim correction (correction d'assiette) ajuste le niveau mesure en tenant compte de l'inclinaison longitudinale du navire. Si le navire est pique sur l'arriere, la surface du liquide n'est pas horizontale et l'ullage mesure au centre n'est pas representatif. Les tables de correction trim sont specifiques a chaque navire."},
      {q:"Que signifie un VCF de 0.9952 pour un cargo de 10 000 m3 bruts ?",opts:["Volume net = 10 048 m3","Volume net = 9 952 m3","Volume net = 9 000 m3","Aucune correction necessaire"],correct:1,expl:"Volume net = Volume brut x VCF = 10 000 x 0.9952 = 9 952 m3 (a 15°C). VCF < 1 signifie que le produit etait plus chaud que 15°C au moment de la mesure, donc le produit occupait plus de volume qu'a 15°C. En refroidissant a 15°C, il occupera 9 952 m3."},
      {q:"Quelle est la difference entre jaugeage 'open' et 'closed' ?",opts:["Open = citerne pleine, closed = citerne vide","Open = tape introduite par le trou de dome ouvert, closed = systeme etanche sans ouverture","Open = jour, closed = nuit","Open = manuel, closed = electronique"],correct:1,expl:"Jaugeage open : tape (ruban) introduite par le trou de dome ouvert, risque d'emission de vapeurs HC (ancien). Jaugeage closed (ou restricted) : systeme de mesure etanche, sans ouverture de la citerne, via un presse-etoupe. Obligatoire pour les bruts tres volatils et requis par MARPOL dans certaines zones."},
      {q:"Le Cargo Record Book (CRB) doit enregistrer pour les operations de jaugeage :",opts:["Seulement le tonnage total","Date, heure, quantite par citerne, ullage, temperature, VCF, masse - tous signes","Uniquement la signature du capitaine","Le prix du cargo"],correct:1,expl:"Le Cargo Record Book exige : date et heure de chaque mesure, ullage et temperature par citerne, VCF applique, volumes brut et net par citerne, masse totale, identification des jaugeurs. Ce document est legalement oppose aux parties dans tout litige sur les quantites."},
      {q:"Qu'est-ce que l'OBQ (On Board Quantity) dans le contexte du jaugeage cargo ?",opts:["La quantite de carburant navire","La quantite residuelle de cargo dans les citernes AVANT chargement","La quantite de ballast","La quantite maximale autorisee"],correct:1,expl:"L'OBQ (On Board Quantity) est la quantite residuelle de cargo (fond de citerne, residus, eau libre) presente dans les citernes AVANT le chargement. Elle est mesuree et enregistree dans l'Ullage Report pre-chargement. L'OBQ est deductible de la quantite chargee selon les termes de la charte-partie."},
      {q:"Pourquoi l'independant cargo surveyor est-il requis pour le jaugeage ?",opts:["Pour reduire les couts","Pour garantir l'impartialite entre armateur et charger - base juridique du BL","Pour surveiller l'equipage","Pour verifier la securite incendie"],correct:1,expl:"L'independant cargo surveyor (inspecteur cargo independant) garantit l'impartialite entre le navire (armateur) et le charger. Ses mesures sont la base legale du Bill of Lading et servent de reference en cas de litige. Certaines compagnies petroliere exigent deux surveyors independants."},
      {q:"Quelle est la correction appliquee pour les tables ASTM 54B ?",opts:["Correction de pression pour les gaz","Correction de temperature pour les hydrocarbures liquides","Correction de salinite pour l'eau de mer","Correction de viscosite"],correct:1,expl:"Les tables ASTM 54B (ou IP 54B) sont des tables de correction de temperature specifiques aux hydrocarbures liquides. Elles donnent le VCF en fonction de la temperature observee et de la densite (ou API gravity) du produit. Reference internationale utilisee dans tous les ports mondiaux."},
      {q:"Lors d'un jaugeage statique, les citernes doivent etre :",opts:["En pompage actif","Au repos depuis au moins 30 minutes (settled)","A temperature maximale","Avec IGS desactive"],correct:1,expl:"Pour un jaugeage statique valide, les citernes doivent etre 'settled' (au repos) depuis au moins 30 minutes (1 heure pour les operations legales) apres tout mouvement de liquide. Cela permet a l'interface liquide de se stabiliser et elimine les vagues et turbulences internes qui fausseraient la mesure."},
      {q:"Le MV Dona Paz / MT Vector (1987) a illustre que le jaugeage non conforme peut :",opts:["Reduire la rentabilite du voyage","Creer une quantite inconnue de produit inflammable avec consequences catastrophiques","Retarder le depart du navire","Causer des problemes douaniers"],correct:1,expl:"Le Vector transportait plus de carburant que declare sur le manifeste cargo (jaugeage frauduleux). En cas de collision, la quantite reelle de produit inflammable etait inconnue, rendant toute intervention impossible. 4 341 morts. Le jaugeage precis est une obligation de securite, pas seulement administrative."},
    ],
    en:[
      {q:"What is ullage in the context of cargo gauging?",opts:["The total tank volume","The distance from the liquid surface to the top of the tank","The weight of the cargo","The cargo temperature"],correct:1,expl:"Ullage (or 'innage' for level measurement) is the distance from the liquid surface to the reference point at the top of the tank (dome or gauge hatch). Formula: Level = Total tank height - Ullage. The larger the ullage, the less the tank is filled."},
      {q:"What is the international reference temperature for net volume calculation?",opts:["0C","15C","20C","60F"],correct:1,expl:"15°C (or 60°F for Anglo-Saxon countries using ASTM) is the international reference temperature per ASTM D1250. All cargo volumes are calculated and declared at this temperature to allow comparison between different ports and climates."},
      {q:"The VCF (Volume Correction Factor) corrects volume to account for:",opts:["Atmospheric pressure","Product temperature","Seawater salinity","Cargo viscosity"],correct:1,expl:"VCF corrects the observed volume (at measurement temperature) to reference volume (at 15°C). VCF = f(temperature, density/API). If T > 15°C, the product is expanded so VCF < 1 (net volume < gross volume). If T < 15°C, VCF > 1."},
      {q:"What is the basic cargo calculation formula?",opts:["Mass = Volume x Pressure","Mass = Net Volume x Density at 15C","Mass = Ullage x Height","Mass = VCF x Temperature"],correct:1,expl:"Mass (MT) = Net volume (m3 at 15°C) x Density at 15°C (t/m3). Net volume is obtained by applying VCF to gross volume. Density is measured at 15°C or corrected via ASTM tables. This formula is the basis of the Bill of Lading."},
      {q:"Which electronic gauge type offers the best legal accuracy per ASTM?",opts:["Float gauge","Radar gauge (GTL)","Servo gauge","Ultrasonic transducer"],correct:2,expl:"The servo gauge (+/- 0.5mm) is the legal precision standard per ASTM D3900. It uses a disc immersed in the product and measures the exerted force. It can also measure density. It is the reference gauge type for legal quantification operations."},
      {q:"What is an Ullage Report?",opts:["The ship's log","Document signed by Chief Officer and independent cargo inspector","Loading plan","PSC inspection report"],correct:1,expl:"The Ullage Report is the official document recording all gauging measurements: ullage, temperature, density per tank, VCF calculations, gross and net volumes, total mass. It is signed by the Chief Officer and independent cargo inspector (surveyor). It forms the basis of the Bill of Lading."},
      {q:"What correction is applied to account for vessel trim during gauging?",opts:["Density correction","Trim correction","List correction","Barometric correction"],correct:1,expl:"Trim correction adjusts the measured level for the vessel's longitudinal inclination. If the vessel is trimmed by stern, the liquid surface is not horizontal and the ullage measured at center is not representative. Trim correction tables are specific to each vessel."},
      {q:"What does a VCF of 0.9952 mean for a cargo of 10,000 m3 gross?",opts:["Net volume = 10,048 m3","Net volume = 9,952 m3","Net volume = 9,000 m3","No correction needed"],correct:1,expl:"Net volume = Gross volume x VCF = 10,000 x 0.9952 = 9,952 m3 (at 15°C). VCF < 1 means the product was warmer than 15°C when measured, so it occupied more volume than at 15°C. When cooled to 15°C, it will occupy 9,952 m3."},
      {q:"What is the difference between 'open' and 'closed' gauging?",opts:["Open = full tank, closed = empty tank","Open = tape through open dome, closed = sealed system without opening","Open = day, closed = night","Open = manual, closed = electronic"],correct:1,expl:"Open gauging: tape introduced through the open dome hatch, risk of HC vapor emission (old method). Closed (or restricted) gauging: sealed measurement system without opening the tank, via a stuffing box. Mandatory for highly volatile crudes and required by MARPOL in certain areas."},
      {q:"The Cargo Record Book (CRB) must record for gauging operations:",opts:["Only total tonnage","Date, time, quantity per tank, ullage, temperature, VCF, mass - all signed","Only captain's signature","Cargo price"],correct:1,expl:"The Cargo Record Book requires: date and time of each measurement, ullage and temperature per tank, VCF applied, gross and net volumes per tank, total mass, gauger identification. This document is legally binding in all quantity disputes."},
      {q:"What is OBQ (On Board Quantity) in cargo gauging?",opts:["The vessel's fuel quantity","Residual cargo in tanks BEFORE loading","The ballast quantity","The maximum permitted quantity"],correct:1,expl:"OBQ is the residual cargo quantity (tank bottoms, residues, free water) present in tanks BEFORE loading. It is measured and recorded in the pre-loading Ullage Report. OBQ is deductible from the loaded quantity per charter party terms."},
      {q:"Why is an independent cargo surveyor required for gauging?",opts:["To reduce costs","To guarantee impartiality between owner and charterer - legal basis of BL","To supervise crew","To verify fire safety"],correct:1,expl:"The independent cargo surveyor guarantees impartiality between the vessel (owner) and the charterer. Their measurements are the legal basis of the Bill of Lading and serve as reference in disputes. Some oil companies require two independent surveyors."},
      {q:"What correction do ASTM 54B tables apply?",opts:["Pressure correction for gases","Temperature correction for liquid hydrocarbons","Salinity correction for seawater","Viscosity correction"],correct:1,expl:"ASTM 54B (or IP 54B) tables are temperature correction tables specific to liquid hydrocarbons. They give VCF as a function of observed temperature and product density (or API gravity). International reference used in all world ports."},
      {q:"For valid static gauging, tanks must be:",opts:["Actively pumping","At rest for at least 30 minutes (settled)","At maximum temperature","With IGS deactivated"],correct:1,expl:"For valid static gauging, tanks must be 'settled' (at rest) for at least 30 minutes (1 hour for legal operations) after any liquid movement. This allows the liquid interface to stabilize and eliminates internal waves and turbulence that would falsify the measurement."},
      {q:"The MV Dona Paz / MT Vector (1987) illustrated that non-compliant gauging can:",opts:["Reduce voyage profitability","Create an unknown quantity of flammable product with catastrophic consequences","Delay vessel departure","Cause customs problems"],correct:1,expl:"Vector was carrying more fuel than declared on the cargo manifest (fraudulent gauging). In case of collision, the actual quantity of flammable product was unknown, making any intervention impossible. 4,341 deaths. Accurate gauging is a safety obligation, not just administrative."},
    ],
    es:[
      {q:"?Que es el ullage en el contexto del sondeo de carga?",opts:["El volumen total del tanque","La distancia entre la superficie del liquido y la parte superior del tanque","El peso de la carga","La temperatura de la carga"],correct:1,expl:"El ullage es la distancia entre la superficie del liquido y el punto de referencia en la parte superior del tanque (cupula o boca de sonda). Formula: Nivel = Altura total tanque - Ullage. Cuanto mayor es el ullage, menos lleno esta el tanque."},
      {q:"?Cual es la temperatura de referencia internacional para el calculo del volumen neto?",opts:["0C","15C","20C","60F"],correct:1,expl:"15°C (o 60°F para paises anglosajones con ASTM) es la temperatura de referencia internacional segun ASTM D1250. Todos los volumenes de carga se calculan y declaran a esta temperatura para permitir la comparacion entre diferentes puertos y climas."},
      {q:"?El VCF (Factor de Correccion de Volumen) corrige el volumen para tener en cuenta:",opts:["La presion atmosferica","La temperatura del producto","La salinidad del agua de mar","La viscosidad de la carga"],correct:1,expl:"El VCF corrige el volumen observado (a la temperatura de medicion) al volumen de referencia (a 15°C). VCF = f(temperatura, densidad/API). Si T > 15°C, el producto esta dilatado por lo que VCF < 1. Si T < 15°C, VCF > 1."},
      {q:"?Cual es la formula basica del calculo de carga?",opts:["Masa = Volumen x Presion","Masa = Volumen neto x Densidad a 15C","Masa = Ullage x Altura","Masa = VCF x Temperatura"],correct:1,expl:"Masa (MT) = Volumen neto (m3 a 15°C) x Densidad a 15°C (t/m3). El volumen neto se obtiene aplicando el VCF al volumen bruto. Esta formula es la base del Bill of Lading."},
      {q:"?Que tipo de medidor electronico ofrece la mejor precision legal segun ASTM?",opts:["Medidor de flotador","Medidor radar (GTL)","Medidor servo","Transductor ultrasonico"],correct:2,expl:"El medidor servo (+/-0.5mm) es el estandar de precision legal segun ASTM D3900. Utiliza un disco sumergido en el producto y mide la fuerza ejercida. Puede tambien medir la densidad. Es el tipo de medidor de referencia para las operaciones de cuantificacion legales."},
      {q:"?Que es un Ullage Report?",opts:["El diario de a bordo","Documento firmado por el Primer Oficial e inspector de carga independiente","Plan de carga","Informe de inspeccion PSC"],correct:1,expl:"El Ullage Report es el documento oficial que registra todas las medidas de sondeo: ullage, temperatura, densidad por tanque, calculos VCF, volumenes bruto y neto, masa total. Es firmado por el Primer Oficial y el inspector de carga independiente (surveyor). Es la base del Bill of Lading."},
      {q:"?Que correccion se aplica para tener en cuenta el asiento del buque durante el sondeo?",opts:["Correccion de densidad","Correccion de asiento (trim correction)","Correccion de escora","Correccion barometrica"],correct:1,expl:"La trim correction ajusta el nivel medido teniendo en cuenta la inclinacion longitudinal del buque. Si el buque esta apopado, la superficie del liquido no es horizontal y el ullage medido en el centro no es representativo."},
      {q:"?Que significa un VCF de 0,9952 para una carga de 10.000 m3 brutos?",opts:["Volumen neto = 10.048 m3","Volumen neto = 9.952 m3","Volumen neto = 9.000 m3","Sin correccion necesaria"],correct:1,expl:"Volumen neto = Volumen bruto x VCF = 10.000 x 0,9952 = 9.952 m3 (a 15°C). VCF < 1 significa que el producto estaba mas caliente que 15°C, por lo que ocupaba mas volumen. Al enfriarse a 15°C, ocupara 9.952 m3."},
      {q:"?Cual es la diferencia entre sondeo 'open' y 'closed'?",opts:["Open = tanque lleno, closed = tanque vacio","Open = cinta por cupula abierta, closed = sistema estanco sin apertura","Open = dia, closed = noche","Open = manual, closed = electronico"],correct:1,expl:"Sondeo open: cinta introducida por la cupula abierta, riesgo de emision de vapores HC. Sondeo closed (restringido): sistema de medicion estanco sin apertura del tanque. Obligatorio para crudos muy volatiles y requerido por MARPOL en ciertas zonas."},
      {q:"?Que debe registrar el Cargo Record Book para operaciones de sondeo?",opts:["Solo el tonelaje total","Fecha, hora, cantidad por tanque, ullage, temperatura, VCF, masa - todo firmado","Solo la firma del capitan","El precio de la carga"],correct:1,expl:"El Cargo Record Book exige: fecha y hora de cada medicion, ullage y temperatura por tanque, VCF aplicado, volumenes bruto y neto por tanque, masa total, identificacion de los sondistas. Documento legalmente vinculante en disputas sobre cantidades."},
      {q:"?Que es el OBQ (On Board Quantity) en el contexto del sondeo de carga?",opts:["La cantidad de combustible del buque","La cantidad residual de carga en los tanques ANTES del cargamento","La cantidad de lastre","La cantidad maxima autorizada"],correct:1,expl:"El OBQ es la cantidad residual de carga (fondos de tanque, residuos, agua libre) presente en los tanques ANTES de cargar. Se mide y registra en el Ullage Report pre-cargamento. El OBQ es deducible de la cantidad cargada segun los terminos de la poliza de fletamento."},
      {q:"?Por que se requiere un inspector de carga independiente para el sondeo?",opts:["Para reducir costes","Para garantizar la imparcialidad entre armador y fletador - base juridica del BL","Para supervisar la tripulacion","Para verificar la seguridad contra incendios"],correct:1,expl:"El inspector de carga independiente garantiza la imparcialidad entre el buque y el fletador. Sus mediciones son la base legal del Bill of Lading y sirven de referencia en disputas. Algunas companias petroleras exigen dos inspectores independientes."},
      {q:"?Que correccion aplican las tablas ASTM 54B?",opts:["Correccion de presion para gases","Correccion de temperatura para hidrocarburos liquidos","Correccion de salinidad para agua de mar","Correccion de viscosidad"],correct:1,expl:"Las tablas ASTM 54B son tablas de correccion de temperatura especificas para hidrocarburos liquidos. Dan el VCF en funcion de la temperatura observada y la densidad (o API gravity). Referencia internacional en todos los puertos mundiales."},
      {q:"Para un sondeo estatico valido, los tanques deben estar:",opts:["En bombeo activo","En reposo durante al menos 30 minutos (settled)","A temperatura maxima","Con IGS desactivado"],correct:1,expl:"Para un sondeo estatico valido, los tanques deben estar en reposo (settled) al menos 30 minutos (1 hora para operaciones legales) tras cualquier movimiento de liquido. Esto permite que la interfaz liquida se estabilice y elimina olas y turbulencias internas."},
      {q:"El MV Dona Paz / MT Vector (1987) ilustro que el sondeo no conforme puede:",opts:["Reducir la rentabilidad del viaje","Crear una cantidad desconocida de producto inflamable con consecuencias catastroficas","Retrasar la salida del buque","Causar problemas aduaneros"],correct:1,expl:"El Vector transportaba mas combustible del declarado en el manifiesto (sondeo fraudulento). En caso de colision, la cantidad real de producto inflamable era desconocida. 4.341 muertos. El sondeo preciso es una obligacion de seguridad, no solo administrativa."},
    ],
    pt:[
      {q:"O que e o ullage no contexto da sondagem de carga?",opts:["O volume total do tanque","A distancia entre a superficie do liquido e o topo do tanque","O peso da carga","A temperatura da carga"],correct:1,expl:"O ullage e a distancia entre a superficie do liquido e o ponto de referencia no topo do tanque (cupula ou tampa de sondagem). Formula: Nivel = Altura total tanque - Ullage. Quanto maior o ullage, menos cheio esta o tanque."},
      {q:"Qual e a temperatura de referencia internacional para o calculo do volume neto?",opts:["0C","15C","20C","60F"],correct:1,expl:"15°C (ou 60°F para paises anglosaxonicos com ASTM) e a temperatura de referencia internacional segundo ASTM D1250. Todos os volumes de carga sao calculados e declarados a esta temperatura para permitir a comparacao entre diferentes portos e climas."},
      {q:"O VCF (Fator de Correcao de Volume) corrige o volume para ter em conta:",opts:["A pressao atmosferica","A temperatura do produto","A salinidade da agua do mar","A viscosidade da carga"],correct:1,expl:"O VCF corrige o volume observado (a temperatura de medicao) para o volume de referencia (a 15°C). VCF = f(temperatura, densidade/API). Se T > 15°C, o produto esta dilatado por isso VCF < 1. Se T < 15°C, VCF > 1."},
      {q:"Qual e a formula basica do calculo de carga?",opts:["Massa = Volume x Pressao","Massa = Volume neto x Densidade a 15C","Massa = Ullage x Altura","Massa = VCF x Temperatura"],correct:1,expl:"Massa (MT) = Volume neto (m3 a 15°C) x Densidade a 15°C (t/m3). O volume neto e obtido aplicando o VCF ao volume bruto. Esta formula e a base do Bill of Lading."},
      {q:"Que tipo de medidor electronico oferece a melhor precisao legal segundo ASTM?",opts:["Medidor de flutuador","Medidor radar (GTL)","Medidor servo","Transdutor ultrasonico"],correct:2,expl:"O medidor servo (+/-0.5mm) e o padrao de precisao legal segundo ASTM D3900. Utiliza um disco imerso no produto e mede a forca exercida. Pode tambem medir a densidade. E o tipo de medidor de referencia para operacoes de quantificacao legais."},
      {q:"O que e um Ullage Report?",opts:["O diario de bordo","Documento assinado pelo Primeiro Oficial e inspector de carga independente","Plano de carga","Relatorio de inspecao PSC"],correct:1,expl:"O Ullage Report e o documento oficial que regista todas as medicoes de sondagem: ullage, temperatura, densidade por tanque, calculos VCF, volumes bruto e neto, massa total. E assinado pelo Primeiro Oficial e inspector de carga independente (surveyor). E a base do Bill of Lading."},
      {q:"Que correcao e aplicada para ter em conta o trim do navio durante a sondagem?",opts:["Correcao de densidade","Correcao de trim (trim correction)","Correcao de escora","Correcao barometrica"],correct:1,expl:"A trim correction ajusta o nivel medido tendo em conta a inclinacao longitudinal do navio. Se o navio esta apopado, a superficie do liquido nao e horizontal e o ullage medido no centro nao e representativo."},
      {q:"O que significa um VCF de 0,9952 para uma carga de 10.000 m3 brutos?",opts:["Volume neto = 10.048 m3","Volume neto = 9.952 m3","Volume neto = 9.000 m3","Sem correcao necessaria"],correct:1,expl:"Volume neto = Volume bruto x VCF = 10.000 x 0,9952 = 9.952 m3 (a 15°C). VCF < 1 significa que o produto estava mais quente que 15°C, portanto ocupava mais volume. Ao arrefecer a 15°C, ocupara 9.952 m3."},
      {q:"Qual e a diferenca entre sondagem 'open' e 'closed'?",opts:["Open = tanque cheio, closed = tanque vazio","Open = fita pela cupula aberta, closed = sistema estanque sem abertura","Open = dia, closed = noite","Open = manual, closed = electronico"],correct:1,expl:"Sondagem open: fita introduzida pela cupula aberta, risco de emissao de vapores HC. Sondagem closed (restrita): sistema de medicao estanque sem abertura do tanque. Obrigatorio para crudes muito volateis e requerido pelo MARPOL em certas zonas."},
      {q:"O que deve registar o Cargo Record Book para operacoes de sondagem?",opts:["So a tonelagem total","Data, hora, quantidade por tanque, ullage, temperatura, VCF, massa - tudo assinado","So a assinatura do capitao","O preco da carga"],correct:1,expl:"O Cargo Record Book exige: data e hora de cada medicao, ullage e temperatura por tanque, VCF aplicado, volumes bruto e neto por tanque, massa total, identificacao dos sondistas. Documento legalmente vinculativo em disputas sobre quantidades."},
      {q:"O que e o OBQ (On Board Quantity) no contexto da sondagem de carga?",opts:["A quantidade de combustivel do navio","A quantidade residual de carga nos tanques ANTES do carregamento","A quantidade de lastro","A quantidade maxima autorizada"],correct:1,expl:"O OBQ e a quantidade residual de carga (fundos de tanque, residuos, agua livre) presente nos tanques ANTES de carregar. E medida e registada no Ullage Report pre-carregamento. O OBQ e dedutivel da quantidade carregada segundo os termos da carta-partida."},
      {q:"Por que e necessario um inspector de carga independente para a sondagem?",opts:["Para reduzir custos","Para garantir a imparcialidade entre armador e fretador - base juridica do BL","Para supervisionar a tripulacao","Para verificar a seguranca contra incendios"],correct:1,expl:"O inspector de carga independente garante a imparcialidade entre o navio e o fretador. As suas medicoes sao a base legal do Bill of Lading e servem de referencia em disputas. Algumas companhias petroliferas exigem dois inspectores independentes."},
      {q:"Que correcao aplicam as tabelas ASTM 54B?",opts:["Correcao de pressao para gases","Correcao de temperatura para hidrocarbonetos liquidos","Correcao de salinidade para agua do mar","Correcao de viscosidade"],correct:1,expl:"As tabelas ASTM 54B sao tabelas de correcao de temperatura especificas para hidrocarbonetos liquidos. Dao o VCF em funcao da temperatura observada e da densidade (ou API gravity). Referencia internacional em todos os portos mundiais."},
      {q:"Para uma sondagem estatica valida, os tanques devem estar:",opts:["Em bombagem activa","Em repouso durante pelo menos 30 minutos (settled)","A temperatura maxima","Com IGS desactivado"],correct:1,expl:"Para uma sondagem estatica valida, os tanques devem estar em repouso (settled) pelo menos 30 minutos (1 hora para operacoes legais) apos qualquer movimento de liquido. Isto permite que a interface liquida se estabilize."},
      {q:"O MV Dona Paz / MT Vector (1987) ilustrou que a sondagem nao conforme pode:",opts:["Reduzir a rentabilidade da viagem","Criar uma quantidade desconhecida de produto inflamavel com consequencias catastroficas","Atrasar a partida do navio","Causar problemas aduaneiros"],correct:1,expl:"O Vector transportava mais combustivel do que declarado no manifesto (sondagem fraudulenta). Em caso de colisao, a quantidade real de produto inflamavel era desconhecida. 4.341 mortos. A sondagem precisa e uma obrigacao de seguranca, nao apenas administrativa."},
    ],
  };
...
export const QUIZ = {
  fr:[
    {q:"Quelle est la formule pour calculer le niveau a partir de l'ullage ?",opts:["Niveau = Ullage - Hauteur","Niveau = Hauteur totale - Ullage","Niveau = Ullage x VCF","Niveau = Densite / Ullage"],correct:1,expl:"Niveau = Hauteur totale de la citerne - Ullage. L'ullage est mesure depuis le sommet de la citerne. Plus l'ullage est petit, plus la citerne est pleine. Ex: citerne 20m, ullage 3m => niveau = 17m."},
    {q:"La temperature de reference pour le volume net selon ASTM est :",opts:["0C","15C","20C","25C"],correct:1,expl:"15°C est la temperature de reference internationale (ASTM D1250 / IP 54B). Tous les volumes cargo sont convertis a 15°C via le VCF pour permettre la comparaison internationale et servir de base legale au Bill of Lading."},
    {q:"Si le VCF est de 0.9980 et le volume brut de 50 000 m3, le volume net est :",opts:["50 100 m3","49 900 m3","45 000 m3","50 000 m3"],correct:1,expl:"Volume net = Volume brut x VCF = 50 000 x 0.9980 = 49 900 m3 (a 15°C). VCF < 1 car le produit etait plus chaud que 15°C lors de la mesure. En refroidissant a 15°C il occupera moins de volume."},
    {q:"Quel document officialise les quantites cargo entre navire et terminal ?",opts:["Manifeste de navigation","Ullage Report signe + Bill of Lading","Cargo Record Book","Plan de chargement"],correct:1,expl:"L'Ullage Report (signe par Chief Officer et surveyor independant) est la base du Bill of Lading (connaissement), document legal qui officialise les quantites echangees. En cas de litige, ce sont les chiffres de l'Ullage Report qui font foi."},
    {q:"Le MV Dona Paz (1987) a montre que le jaugeage frauduleux du Vector :",opts:["A retarde le depart","A cree une quantite inconnue de produit inflammable aggravant la catastrophe","A reduit la charge du navire","A cause la panne du moteur"],correct:1,expl:"Le Vector avait sous-declare les quantites de carburant (sondeo frauduleux). La quantite reelle d'hydrocarbures inflamambles etait donc inconnue. Lors de la collision avec le Dona Paz, l'incendie catastrophique a resulte de cette quantite non connue, rendant toute intervention impossible. 4 341 morts."},
  ],
  en:[
    {q:"What is the formula to calculate level from ullage?",opts:["Level = Ullage - Height","Level = Total height - Ullage","Level = Ullage x VCF","Level = Density / Ullage"],correct:1,expl:"Level = Total tank height - Ullage. Ullage is measured from the tank top. The smaller the ullage, the fuller the tank. Example: 20m tank, 3m ullage => level = 17m."},
    {q:"The reference temperature for net volume per ASTM is:",opts:["0C","15C","20C","25C"],correct:1,expl:"15°C is the international reference temperature (ASTM D1250 / IP 54B). All cargo volumes are converted to 15°C via VCF to allow international comparison and serve as legal basis for the Bill of Lading."},
    {q:"If VCF is 0.9980 and gross volume is 50,000 m3, net volume is:",opts:["50,100 m3","49,900 m3","45,000 m3","50,000 m3"],correct:1,expl:"Net volume = Gross volume x VCF = 50,000 x 0.9980 = 49,900 m3 (at 15°C). VCF < 1 because the product was warmer than 15°C when measured. Cooling to 15°C it will occupy less volume."},
    {q:"Which document officially records cargo quantities between vessel and terminal?",opts:["Navigation manifest","Signed Ullage Report + Bill of Lading","Cargo Record Book","Loading plan"],correct:1,expl:"The Ullage Report (signed by Chief Officer and independent surveyor) is the basis of the Bill of Lading, the legal document officializing exchanged quantities. In disputes, the Ullage Report figures are the legal reference."},
    {q:"The MV Dona Paz (1987) showed that the Vector's fraudulent gauging:",opts:["Delayed departure","Created an unknown quantity of flammable product worsening the disaster","Reduced the vessel's load","Caused engine failure"],correct:1,expl:"The Vector had under-declared fuel quantities (fraudulent gauging). The actual quantity of flammable hydrocarbons was unknown. When the collision with Dona Paz occurred, the catastrophic fire resulted from this unknown quantity, making any intervention impossible. 4,341 deaths."},
  ],
  es:[
    {q:"?Cual es la formula para calcular el nivel a partir del ullage?",opts:["Nivel = Ullage - Altura","Nivel = Altura total - Ullage","Nivel = Ullage x VCF","Nivel = Densidad / Ullage"],correct:1,expl:"Nivel = Altura total del tanque - Ullage. El ullage se mide desde la parte superior del tanque. Cuanto menor es el ullage, mas lleno esta el tanque. Ej: tanque 20m, ullage 3m => nivel = 17m."},
    {q:"La temperatura de referencia para el volumen neto segun ASTM es:",opts:["0C","15C","20C","25C"],correct:1,expl:"15°C es la temperatura de referencia internacional (ASTM D1250 / IP 54B). Todos los volumenes de carga se convierten a 15°C via el VCF para permitir la comparacion internacional y servir de base legal al Bill of Lading."},
    {q:"Si el VCF es 0,9980 y el volumen bruto 50.000 m3, el volumen neto es:",opts:["50.100 m3","49.900 m3","45.000 m3","50.000 m3"],correct:1,expl:"Volumen neto = Volumen bruto x VCF = 50.000 x 0,9980 = 49.900 m3 (a 15°C). VCF < 1 porque el producto estaba mas caliente que 15°C al medir. Al enfriarse a 15°C ocupara menos volumen."},
    {q:"?Que documento oficializa las cantidades de carga entre buque y terminal?",opts:["Manifiesto de navegacion","Ullage Report firmado + Bill of Lading","Cargo Record Book","Plan de carga"],correct:1,expl:"El Ullage Report (firmado por Primer Oficial e inspector independiente) es la base del Bill of Lading, el documento legal que oficializa las cantidades intercambiadas. En disputas, las cifras del Ullage Report son la referencia legal."},
    {q:"El MV Dona Paz (1987) demostro que el sondeo fraudulento del Vector:",opts:["Retraso la salida","Creo una cantidad desconocida de producto inflamable agravando la catastrofe","Redujo la carga del buque","Causo la averia del motor"],correct:1,expl:"El Vector habia subdeclarado las cantidades de combustible (sondeo fraudulento). La cantidad real de hidrocarburos inflamables era desconocida. Al colisionar con el Dona Paz, el incendio catastrofico resulto de esta cantidad desconocida. 4.341 muertos."},
  ],
  pt:[
    {q:"Qual e a formula para calcular o nivel a partir do ullage?",opts:["Nivel = Ullage - Altura","Nivel = Altura total - Ullage","Nivel = Ullage x VCF","Nivel = Densidade / Ullage"],correct:1,expl:"Nivel = Altura total do tanque - Ullage. O ullage e medido a partir do topo do tanque. Quanto menor o ullage, mais cheio esta o tanque. Ex: tanque 20m, ullage 3m => nivel = 17m."},
    {q:"A temperatura de referencia para o volume neto segundo ASTM e:",opts:["0C","15C","20C","25C"],correct:1,expl:"15°C e a temperatura de referencia internacional (ASTM D1250 / IP 54B). Todos os volumes de carga sao convertidos a 15°C via VCF para permitir a comparacao internacional e servir de base legal ao Bill of Lading."},
    {q:"Se o VCF e 0,9980 e o volume bruto 50.000 m3, o volume neto e:",opts:["50.100 m3","49.900 m3","45.000 m3","50.000 m3"],correct:1,expl:"Volume neto = Volume bruto x VCF = 50.000 x 0,9980 = 49.900 m3 (a 15°C). VCF < 1 porque o produto estava mais quente que 15°C ao medir. Ao arrefecer a 15°C ocupara menos volume."},
    {q:"Que documento oficializa as quantidades de carga entre navio e terminal?",opts:["Manifesto de navegacao","Ullage Report assinado + Bill of Lading","Cargo Record Book","Plano de carga"],correct:1,expl:"O Ullage Report (assinado pelo Primeiro Oficial e inspector independente) e a base do Bill of Lading, o documento legal que oficializa as quantidades trocadas. Em disputas, os valores do Ullage Report sao a referencia legal."},
    {q:"O MV Dona Paz (1987) mostrou que a sondagem fraudulenta do Vector:",opts:["Atrasou a partida","Criou uma quantidade desconhecida de produto inflamavel agravando a catastrofe","Reduziu a carga do navio","Causou a avaria do motor"],correct:1,expl:"O Vector tinha subdeclarado as quantidades de combustivel (sondagem fraudulenta). A quantidade real de hidrocarbonetos inflamaveis era desconhecida. Na colisao com o Dona Paz, o incendio catastrofico resultou desta quantidade desconhecida. 4.341 mortos."},
  ],
};
...
function QuizComp({ questions, t, lang, onComplete }) {
  ...
  const handleNext = () => {
    const fs = score+(sel===q.correct?1:0);
...
// MAIN EXPORT
// ══════════════════════════════════════
export default function LessonE6_L6({ lang="fr", onBack=()=>{}, onComplete=()=>{}, onNext=()=>{}, onQuizScored=(score:number,maxScore:number)=>{} }) {
  const t = T[lang]||T.fr;
        ...
        <div style={{maxWidth:480,margin:"0 auto"}}>
          ...
          {phase==="content"&&<>
            ...
            <button disabled={!bankDone} onClick={()=>{if(bankDone)setPhase("quiz");}}
              style={{opacity:bankDone?1:0.45,cursor:bankDone?"pointer":"not-allowed",width:"100%",padding:"17px 0",border:"none",borderRadius:16,background:`linear-gradient(135deg,${C.gauge},${C.gold})`,fontFamily:"'Cinzel',serif",fontSize:15,fontWeight:700,letterSpacing:2,color:C.white,boxShadow:"0 10px 36px rgba(38,198,218,0.3)",marginTop:4}}>
              {t.startQuiz}
            </button>
          ...
          {phase==="quiz"&&<>
            ...
            <QuizComp questions={quiz} t={t} lang={lang} onComplete={s=>{setQuizScore(s);onQuizScored(s,quiz.length);setTimeout(()=>setPhase("done"),400);}}/>
          </>}
