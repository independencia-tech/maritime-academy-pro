...
// PALETTE - shared across all Safety lessons
export const C = {
...
export function QuizComp({questions,t,onComplete}){
  ...
  const next=()=>{if(cur<questions.length-1){setCur(c=>c+1);setSel(null);setAnswered(false);}else{setDone(true);onComplete(score);}};
...
export function QuestionBank({ lang, t, questions, onComplete }) {
  ...
  if(!started) return (
    <div style={{textAlign:"center",padding:"10px 0"}}>
      ...
      <button onClick={()=>setStarted(true)} style={{width:"100%",padding:"13px 0",border:"none",borderRadius:14,background:`linear-gradient(135deg,${C.purple},${C.red})`,fontFamily:"'Cinzel',serif",fontSize:13,fontWeight:700,letterSpacing:2,color:C.white,cursor:"pointer"}}>{t.startBank}</button>
    </div>
  );
  if(done) return <div style={{textAlign:"center",padding:"20px 0"}}><div style={{fontSize:48,marginBottom:8}}>{score>=Math.ceil(questions.length*0.8)?"🏆":score>=Math.ceil(questions.length*0.5)?"🎖️":"📚"}</div><div style={{fontFamily:"'Cinzel',serif",fontSize:20,color:C.white,marginBottom:4}}>{score}/{questions.length}</div><div style={{fontSize:14,color:C.gold2}}>{Math.round(score/questions.length*100)}%</div></div>;
  return (
    <div>
