...
function QuizTab({ lang, onComplete }:{ lang:string; onComplete:(xp:number,score:number,maxScore:number)=>void }) {
  ...
  if(done) return (
    <div style={{padding:"24px 14px"}}>
      ...
      <button onClick={()=>onComplete(xp,score,quiz.length)} style={{width:"100%",padding:"15px 0",border:"none",borderRadius:14,background:"linear-gradient(135deg,#f97316,#c9922a)",fontFamily:"'Cinzel',serif",fontSize:14,fontWeight:700,letterSpacing:2,color:"#060e1a",cursor:"pointer"}}>🛡️ {l.finish}</button>
...
export default function LessonE3_L5({ lang="fr", onBack, onComplete, onQuizScored }:{ lang?:string; onBack:()=>void; onComplete?:(xp?:number)=>void; onQuizScored?:(score:number,maxScore:number)=>void; }) {
  const t=T[lang]||T.fr;
