...
export function MusicProvider({ children }) {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (!audioRef.current) {
      const a = new Audio("/mixkit-sea-waves-with-birds-loop-1185.wav");
      a.loop = true;
      a.volume = 0.5;
      audioRef.current = a;
    }
  ...
  useEffect(() => {
    ...
    const tryResume = () => {
      const a = audioRef.current;
      if (a && enabled && !muted && a.paused) {
        a.play().catch(() => {});
      }
    };
    window.addEventListener("pointerdown", tryResume, { once: true, capture: true });
    window.addEventListener("touchstart", tryResume, { once: true, capture: true });
    return () => {
      window.removeEventListener("pointerdown", tryResume, { capture: true });
      window.removeEventListener("touchstart", tryResume, { capture: true });
    };
  }, [enabled, muted]);
  ...
  const enable = () => {
    ...
    setEnabled(true);
    // Tentative de lecture directe dans le geste utilisateur (politique autoplay mobile)
    if (audioRef.current) {
