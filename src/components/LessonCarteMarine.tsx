...
function QuestionBank({ lang }) {
  ...
  if(done) return (
    <div style={{textAlign:"center",padding:"20px 0"}}>
      ...
      <div style={{fontFamily:"'Cinzel',serif",fontSize:22,color:C.white,marginBottom:6}}>{score}/{BANK.length}</div>
      <div style={{fontSize:14,color:C.gold2,marginBottom:4}}>{Math.round(score/BANK.length*100)}%</div>
    ...
    <div>
      ...
      {answered&&<>
        ...
        <button onClick={next} style={{width:"100%",padding:"12px 0",border:"none",borderRadius:12,background:`linear-gradient(135deg,${C.purple},${C.blue})`,fontFamily:"'Cinzel',serif",fontSize:12,fontWeight:700,letterSpacing:2,color:C.white,cursor:"pointer"}}>
          {cur<BANK.length-1?(lang==="fr"?"SUIVANT →":lang==="es"?"SIGUIENTE →":lang==="pt"?"PRÓXIMO →":"NEXT →"):(lang==="fr"?"TERMINER":"FINISH")}
        </button>
      </>}
    </div>
  );
