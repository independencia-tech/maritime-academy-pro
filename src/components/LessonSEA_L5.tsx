...
function GBMDiagramSVG({ lang }) {
  ...
  const pts=[
    ...
    {id:"M",cx:110,cy:62,r:7,color:C.meta,
      ...
      desc:lbl("Point virtuel : intersection de la verticale de la poussee gitee et de l'axe initial. GM positif (G sous M) = STABLE. GM nul = NEUTRE. GM negatif (G sur M) = INSTABLE et risque chavirement. Seuil reglementaire standard : GM min = 0,15 m.",
        "Virtual point: intersection of heeled buoyancy vertical and initial axis. Positive GM (G below M) = STABLE. Zero GM = NEUTRAL. Negative GM (G above M) = UNSTABLE, capsize risk. Standard regulatory threshold: min GM = 0.15 m.",
        "Punto virtual. GM positivo (G bajo M) = ESTABLE. GM nulo = NEUTRO. GM negativo = INESTABLE. Umbral reglamentario estandar: GM min = 0,15 m.",
        "Ponto virtual. GM positivo (G abaixo de M) = ESTAVEL. GM nulo = NEUTRO. GM negativo = INSTAVEL. Limiar regulamentar padrao: GM min = 0,15 m.")},
  ];
...
function StabilityStatesSVG({ lang }) {
  ...
  const states=[
    ...
    {id:"tender",icon:"🌊",color:C.grav,gY:104,mY:110,
      ...
      desc:lbl("GM faiblement positif. Gite lente et longue. Risque si evenement supplementaire. Seuil reglementaire standard : GM minimum = 0,15 m. Solution : ajouter ballast bas.",
        "Weakly positive GM. Slow long roll. Risk if additional event. Standard regulatory threshold: min GM = 0.15 m. Solution: add low ballast.",
        "GM debilmente positivo. Escora lenta y prolongada. Umbral reglamentario estandar: GM minimo = 0,15 m. Solucion: anadir lastre bajo.",
        "GM debilmente positivo. Banda lenta e prolongada. Limiar regulamentar padrao: GM minimo = 0,15 m. Solucao: adicionar lastro baixo.")},
  ];
    ...
    <div>
      <div style={{display:"flex",gap:5,marginBottom:10}}>
        {[
          ...
          {id:"imo",label:"IS Code 2008",color:C.green},
...
// answer-for-answer. Like d6-l4, all 14 questions already had complete
// 4-language expl text in the source — no fallback anomaly here.
// ══════════════════════════════════════
export const BANK = {
  fr: [
    {q:"Qu'est-ce que la flottabilite et le principe d'Archimede ?",opts:["La stabilite du navire","Force ascendante = poids du volume d'eau deplace. Navire flotte si poussee = poids total. P = rho x g x V immerse.","Le poids de l'ancre","La courbe GZ"],correct:1,expl:"Principe d'Archimede : toute corps plonge dans un fluide recoit une poussee verticale ascendante egale au poids du volume de fluide deplace. Pour un navire : Poussee (tonnes) = deplacement (tonnes). Le navire flotte car sa forme lui permet de deplaocer un volume d'eau dont le poids est egal au sien."},
    {q:"Quelle est la formule de GM et que signifient K, G et M ?",opts:["GM = KG x KM","GM = KM - KG. K = quille (bas), G = centre de gravite (masses navire), M = metacentre (virtuel, stabilite). GM > 0 = stable.","GM = KG - KM","GM = G / M"],correct:1,expl:"GM = KM - KG est la formule fondamentale de la stabilite initiale. K = quille = point de reference. KG = distance de la quille au centre de gravite G (augmente si on charge en hauteur). KM = distance quille au metacentre M (depend de la forme de la coque). Si GM > 0 : G sous M = stable. Si GM < 0 : G au-dessus de M = instable."},
    {q:"Qu'est-ce que le bras de levier GZ ?",opts:["Le poids de G","Distance horizontale entre G et la verticale de B quand le navire gite. Si GZ > 0 : couple redressant. Si GZ < 0 : couple chavirant. GZ max reglementaire >= 0,20 m a >= 30.","La profondeur d'immersion","La distance KM"],correct:1,expl:"GZ est le bras du couple redressant (ou chavirant). Plus GZ est grand, plus la force redressante est importante. La courbe GZ en fonction de la gite donne toute l'information de stabilite : GM initial (pente a l'origine), GZ maximum, angle de chavirement. Le seuil reglementaire standard exige GZ max >= 0,20 m a un angle >= 30 degres."},
    {q:"Quels sont les criteres minimaux du IS Code pour la stabilite initiale ?",opts:["GM >= 1 m uniquement","GM >= 0,15 m, GZ max >= 0,20 m a >= 30 deg, angle chavirement >= 25 apres max, aires 0-30 >= 0,055 / 0-40 >= 0,090 / 30-40 >= 0,030 m.rad","GZ max >= 0,50 m uniquement","GM >= 0,50 m et GZ >= 0,30 m"],correct:1,expl:"IS Code 2008 (MSC.267(85)) : (1) GM initial >= 0,15 m. (2) GZ max >= 0,20 m a angle >= 30. (3) Angle de chavirement >= 25 apres le max GZ. (4) Aire 0-30 >= 0,055 m.rad. (5) Aire 0-40 >= 0,090 m.rad. (6) Aire 30-40 >= 0,030 m.rad. Tous ces criteres doivent etre satisfaits dans toutes les conditions de chargement prevues."},
    {q:"Comment l'effet de surface libre se calcule-t-il ?",opts:["GG' = i x densite liquide / Deplacement navire (tonnes)","GG' = GM x sin(gite)","GG' = KM - KG","GG' = longueur x largeur citerne"],correct:0,expl:"Correction surface libre : GG' = (i x gamma_liquide) / Delta, ou i = moment d'inertie de la surface libre (l x b^3 / 12 pour un rectangle en m4), gamma_liquide = densite du liquide (eau = 1,025, gasoil = 0,85, etc.), Delta = deplacement total du navire en tonnes. GM corrige = GM initial - GG'. Diviser une citerne de largeur b en deux (largeur b/2 chacune) reduit i par 4, donc GG' par 4."},
    {q:"Quelle est la difference entre un navire 'raide' et un navire 'mou' ?",opts:["Raide = plus lourd","Raide (GM tres positif) = mouvements brusques, inconfort, fatigue structures. Mou (GM faible) = gite lente et longue, risque si evenement supplementaire","Raide = instable","Il n'y a pas de difference pratique"],correct:1,expl:"Navire raide (GM > 2m) : se redresse tres vite avec des mouvements secs et violents. Risques : inconfort equipage, fatigue acceleree des structures et amarrages, deplacements de cargaison, acces dangereux. Navire mou (0 < GM < 0,2m) : gite lente et prend du temps a se redresser. Risques : si un evenement supplementaire (vague de travers, depl. cargaison, envahissement) survient pendant la gite, le navire peut ne pas se redresser. L'ideal : GM autour de 0,5 - 1,5m."},
    {q:"Qu'est-ce que le franc-bord et pourquoi est-il lie a la stabilite ?",opts:["La hauteur des mats","Distance waterline - pont principal. Determine la reserve de flottabilite. Grand franc-bord = angle de chavirement eleve. Plimsoll fixe le minimum legal par zone.","La couleur de la coque","La distance entre les ponts"],correct:1,expl:"Le franc-bord est la distance entre la ligne de flottaison et le pont de franc-bord (pont etanche). Il determine la RESERVE DE FLOTTABILITE — plus il est grand, plus le navire peut giter avant que l'eau embarque sur le pont, augmentant l'angle de chavirement. La ligne de charge Plimsoll fixe le franc-bord minimum legal selon la zone (Tropical, Estival, Hivernal, WNA) et la densite de l'eau (eau douce FW). Un franc-bord trop faible = angle de chavirement reduit = danger."},
    {q:"Qu'est-ce que l'experience de stabilite (inclining experiment) ?",opts:["Un test de vitesse","Essai pour determiner experimentalement KG et GM. Masse p deplacee distance d -> mesure gite theta -> GM = (p x d) / (Delta x tan theta). Obligatoire a la construction.","Un test de ballast","Un exercice d'urgence"],correct:1,expl:"L'experience de stabilite (inclining experiment) est un essai obligatoire realise a la construction et apres modifications majeures du navire. Methode : deplacer une masse connue p d'une distance transversale d connue. Mesurer l'angle de gite theta resultant. Calculer : GM = (p x d) / (Delta x tan theta). Puis KG = KM - GM. Donne les parametres de base pour le calcul de stabilite dans toutes les conditions de chargement."},
    {q:"Quelle est la difference entre stabilite initiale et stabilite a grande inclinaison ?",opts:["Pas de difference","Initiale (< 10-15 deg) : caracterisee par GM seul, GZ = GM x sin theta. Grande inclinaison (> 15 deg) : il faut la courbe GZ complete, le metacentre se deplace, la forme de la coque joue un role majeur.","Grande inclinaison = quand le GM est eleve","La stabilite initiale est plus importante"],correct:1,expl:"Stabilite initiale (angles < 10-15 deg) : entierement decrite par GM. A ces angles, GZ = GM x sin theta. Le metacentre M reste fixe. Stabilite a grande inclinaison : le metacentre se deplace car la forme du volume immerse change de facon non lineaire. Il faut la courbe GZ complete. La forme de la coque (accastillage, etrave, tableau arriere) joue un role majeur. Un navire peut avoir un bon GM initial mais chavirer a 35 deg si la courbe GZ diminue trop vite."},
    {q:"Qu'est-ce que le ballast et quel est son role dans la stabilite ?",opts:["La cargaison payante","Eau de mer pompee dans des citernes deidiees pour ajuster le tirant d'eau, l'assiette et la stabilite. Remplir citernes basses -> abaisse G -> augmente GM. MARPOL encadre les eaux de ballast (especes invasives).","Le carburant","L'eau de lavage des ponts"],correct:1,expl:"Le ballast est de l'eau de mer pompee dans des citernes dediees quand le navire est en lest (sans cargaison) ou quand la stabilite doit etre amelioree. Remplir les citernes de ballast basses abaisse le centre de gravite G -> augmente GM -> ameliore la stabilite. Attention : MARPOL Annexe V et la convention internationale de gestion des eaux de ballast (2004) : les eaux de ballast doivent etre traitees avant rejet pour eviter l'introduction d'especes invasives."},
    {q:"Qu'est-ce que l'assiette (trim) et comment affecte-t-elle la stabilite ?",opts:["La vitesse du navire","Difference tirant eau AR - tirant eau AV. Assiette positive (poupe enfoncee) = normale et favorable propulsion. Assiette excessive modifie KM et peut reduire la stabilite. Toujours respecter les limites du plan de chargement.","La hauteur de la cargaison","Le poids total du navire"],correct:1,expl:"L'assiette (trim) = tirant d'eau arriere - tirant d'eau avant. Une assiette positive (poupe plus enfoncee) est normale et ameliore l'efficacite de l'helice et du gouvernail. Une assiette excessive modifie la forme du volume immerse, ce qui deplace KM et peut affecter GM. Des assiettes trop importantes peuvent aussi reduire l'angle de chavirement. Toujours respecter les limites d'assiette prescrites dans le plan de chargement/stability booklet."},
    {q:"Qu'est-ce que la stabilite dynamique et comment se mesure-t-elle ?",opts:["Le GM au moment du lancement","Energie totale de redressement = aire sous la courbe GZ de 0 a l'angle de chavirement (m.rad). Represente la capacite a resister aux perturbations dynamiques (vagues, rafales). Plus l'aire est grande, plus le navire est sur.","La vitesse de rotation du navire","La hauteur de la vague"],correct:1,expl:"La stabilite dynamique est l'energie totale disponible pour le redressement. Elle est egale a l'aire sous la courbe GZ entre 0 et l'angle de chavirement (en m.rad). Cette energie represente le travail que le navire peut faire contre les forces destabilisantes (vagues de travers, rafales de vent). Le IS Code specifie des aires minimales : 0-30 >= 0,055 m.rad, 0-40 >= 0,090 m.rad, 30-40 >= 0,030 m.rad. Un navire avec une grande aire sous la courbe GZ resiste mieux aux conditions meteorologiques severes."},
    {q:"Comment les cloisons longitudinales dans les citernes reduisent-elles l'effet de surface libre ?",opts:["Elles augmentent le volume des citernes","Diviser citerne largeur b en 2 x (b/2) : moment d'inertie i passe de l b^3/12 a 2 x l(b/2)^3/12 = l b^3/48. Resultat : i divise par 4 -> GG' divise par 4.","Elles reduisent le poids du liquide","Elles augmentent KM"],correct:1,expl:"La formule cle : pour une citerne rectangulaire, moment d'inertie i = (longueur x largeur^3) / 12. L'effet de surface libre GG' est proportionnel a i. En ajoutant une cloison longitudinale centrale qui divise la largeur b par 2 : nouvelle i totale = 2 x (l x (b/2)^3) / 12 = l x b^3 / 48. Soit 4 fois moins que la citerne originale (l x b^3 / 12). Donc GG' est divise par 4. C'est pourquoi les navires petroliers et chimiquiers ont des citernes etroites multiples."},
    {q:"Qu'est-ce que l'angle de chavirement (vanishing stability) et quel est le critere du IS Code ?",opts:["L'angle maximal autorise de navigation","Angle au-dela duquel GZ redevient nul et le navire ne peut plus se redresser. IS Code : angle >= 25 degres apres le maximum de GZ. Angle de chavirement faible (< 40 deg) = navire dangereusement instable.","L'angle de gite permanent","L'angle d'assiette maximale"],correct:1,expl:"L'angle de chavirement (vanishing stability angle) est l'angle de gite auquel GZ redevient nul apres son maximum positif. Au-dela, le navire ne peut plus se redresser seul - il chavire. Pour un navire de charge conventionnel, le IS Code 2008 exige que cet angle soit d'au moins 25 degres apres le maximum de GZ. Un angle de chavirement eleve (> 60-70 deg) indique un navire tres sur, avec une grande reserve de stabilite. La courbe GZ et cet angle sont calcules par le logiciel de stabilite pour chaque condition de chargement."},
  ],
  en: [
    {q:"What is buoyancy and Archimedes' principle?",opts:["Vessel stability","Upward force = weight of displaced water volume. Vessel floats if buoyancy = total weight. B = rho x g x V submerged.","Anchor weight","The GZ curve"],correct:1,expl:"Archimedes: any body in a fluid receives an upward force equal to the weight of displaced fluid. For a vessel: Buoyancy (tonnes) = displacement (tonnes). The vessel floats because its shape displaces a water volume whose weight equals its own."},
    {q:"What is the GM formula and what do K, G and M mean?",opts:["GM = KG x KM","GM = KM - KG. K = keel (bottom), G = centre of gravity (vessel masses), M = metacentre (virtual, stability). GM > 0 = stable.","GM = KG - KM","GM = G / M"],correct:1,expl:"GM = KM - KG is the fundamental initial stability formula. K = keel = reference point. KG = keel to G distance (increases if loaded high). KM = keel to metacentre (depends on hull form). GM > 0: G below M = stable. GM < 0: G above M = unstable."},
    {q:"What is the righting lever GZ?",opts:["Weight of G","Horizontal distance between G and vertical through B when heeled. If GZ > 0: righting couple. If GZ < 0: capsizing couple. Regulatory max GZ >= 0.20 m at >= 30 deg.","The draught","The KM distance"],correct:1,expl:"GZ is the righting (or capsizing) couple arm. Larger GZ = greater righting force. The GZ curve vs heel gives all stability info: initial GM (slope at origin), max GZ, angle of vanishing stability. The standard regulatory threshold requires max GZ >= 0.20 m at angle >= 30 deg."},
    {q:"What are the IS Code minimum criteria for initial stability?",opts:["GM >= 1 m only","GM >= 0.15 m, max GZ >= 0.20 m at >= 30 deg, vanishing angle >= 25 past max, areas 0-30 >= 0.055 / 0-40 >= 0.090 / 30-40 >= 0.030 m.rad","Max GZ >= 0.50 m only","GM >= 0.50 m and GZ >= 0.30 m"],correct:1,expl:"IS Code 2008 (MSC.267(85)): (1) Initial GM >= 0.15 m. (2) Max GZ >= 0.20 m at >= 30 deg. (3) Vanishing angle >= 25 past GZ max. (4) Area 0-30 >= 0.055 m.rad. (5) Area 0-40 >= 0.090 m.rad. (6) Area 30-40 >= 0.030 m.rad. All criteria must be met in all planned loading conditions."},
    {q:"How is the free surface effect calculated?",opts:["GG' = i x liquid density / Vessel displacement (tonnes)","GG' = GM x sin(heel)","GG' = KM - KG","GG' = tank length x width"],correct:0,expl:"Free surface correction: GG' = (i x gamma_liquid) / Delta, where i = free surface moment of inertia (l x b^3 / 12 for rectangle in m4), gamma_liquid = liquid density (seawater = 1.025, diesel = 0.85 etc.), Delta = total vessel displacement in tonnes. Corrected GM = initial GM - GG'. Dividing a tank of width b into two (each width b/2) reduces i by 4, thus GG' by 4."},
    {q:"What is the difference between a 'stiff' and a 'tender' vessel?",opts:["Stiff = heavier","Stiff (very positive GM) = sudden movements, discomfort, structural fatigue. Tender (low GM) = slow long roll, risk if additional event","Stiff = unstable","No practical difference"],correct:1,expl:"Stiff vessel (GM > 2m): rights itself very fast with violent movements. Risks: crew discomfort, accelerated structural fatigue, cargo shifts, dangerous access. Tender vessel (0 < GM < 0.2m): heels slowly and takes time to right. Risks: if additional event (beam sea, cargo shift, flooding) occurs during the heel, vessel may not right itself. Ideal: GM around 0.5 - 1.5m."},
    {q:"What is freeboard and why is it linked to stability?",opts:["Mast height","Waterline to main deck distance. Determines reserve buoyancy. Large freeboard = high vanishing angle. Plimsoll fixes legal minimum by zone.","Hull color","Distance between decks"],correct:1,expl:"Freeboard is the distance between the waterline and the freeboard deck (weathertight deck). It determines RESERVE OF BUOYANCY — greater freeboard means the vessel can heel more before water comes on deck, increasing the vanishing angle. The Plimsoll load line fixes minimum legal freeboard by zone (Tropical, Summer, Winter, WNA) and water density (fresh water FW). Too little freeboard = reduced vanishing angle = danger."},
    {q:"What is the inclining experiment?",opts:["A speed test","Test to experimentally determine KG and GM. Mass p shifted distance d -> measured heel theta -> GM = (p x d) / (Delta x tan theta). Mandatory at construction.","A ballast test","An emergency drill"],correct:1,expl:"The inclining experiment is a mandatory test performed at vessel construction and after major modifications. Method: move known mass p a known transverse distance d. Measure resulting heel angle theta. Calculate: GM = (p x d) / (Delta x tan theta). Then KG = KM - GM. Gives base parameters for stability calculations in all loading conditions."},
    {q:"What is the difference between initial stability and large-angle stability?",opts:["No difference","Initial (< 10-15 deg): characterised by GM alone, GZ = GM x sin theta. Large angle (> 15 deg): full GZ curve needed, metacentre moves, hull form plays major role.","Large angle = when GM is high","Initial stability is more important"],correct:1,expl:"Initial stability (< 10-15 deg): fully described by GM. At these angles GZ = GM x sin theta. Metacentre M stays fixed. Large-angle stability: metacentre moves as submerged volume shape changes non-linearly. Full GZ curve needed. Hull form (superstructure, bow, stern) plays major role. A vessel can have good initial GM but capsize at 35 deg if GZ curve falls too quickly."},
    {q:"What is ballast and its role in stability?",opts:["Paying cargo","Seawater pumped into dedicated tanks to adjust draught, trim and stability. Fill low tanks -> lowers G -> increases GM. MARPOL regulates ballast water (invasive species).","The fuel","Deck wash water"],correct:1,expl:"Ballast is seawater pumped into dedicated tanks when vessel is in ballast (no cargo) or when stability needs improving. Filling low ballast tanks lowers G -> increases GM -> improves stability. Note: MARPOL Annex V and the international Ballast Water Management Convention (2004): ballast water must be treated before discharge to prevent invasive species introduction."},
    {q:"What is trim and how does it affect stability?",opts:["Vessel speed","Aft draught minus forward draught. Positive trim (stern deeper) = normal and good for propulsion. Excessive trim changes KM and can reduce stability. Always stay within loading plan limits.","Cargo height","Total vessel weight"],correct:1,expl:"Trim = aft draught - forward draught. Positive trim (stern deeper) is normal and improves propeller and rudder efficiency. Excessive trim changes submerged volume shape, moving KM and potentially affecting GM. Large trims can also reduce vanishing angle. Always respect trim limits prescribed in loading plan/stability booklet."},
    {q:"What is dynamical stability and how is it measured?",opts:["GM at launch","Total righting energy = area under GZ curve from 0 to vanishing angle (m.rad). Represents capacity to resist dynamic disturbances (waves, gusts). Larger area = safer vessel.","Vessel rotation speed","Wave height"],correct:1,expl:"Dynamical stability is the total available righting energy, equal to the area under the GZ curve from 0 to the vanishing angle (m.rad). This energy represents the work the vessel can do against destabilising forces (beam seas, wind gusts). The IS Code specifies minimum areas: 0-30 >= 0.055 m.rad, 0-40 >= 0.090 m.rad, 30-40 >= 0.030 m.rad. A vessel with large GZ curve area resists severe weather better."},
    {q:"How do longitudinal tank divisions reduce the free surface effect?",opts:["They increase tank volume","Divide tank width b into 2 x (b/2): inertia i goes from l b^3/12 to 2 x l(b/2)^3/12 = l b^3/48. Result: i divided by 4 -> GG' divided by 4.","They reduce liquid weight","They increase KM"],correct:1,expl:"Key formula: for a rectangular tank, moment of inertia i = (length x width^3) / 12. Free surface effect GG' is proportional to i. Adding a central longitudinal partition dividing width b by 2: new total i = 2 x (l x (b/2)^3) / 12 = l x b^3 / 48. Four times less than original tank (l x b^3 / 12). So GG' is divided by 4. This is why tankers and chemical carriers have multiple narrow tanks."},
    {q:"What is the angle of vanishing stability and what is the IS Code criterion?",opts:["Maximum allowed navigation angle","Angle beyond which GZ returns to zero and vessel can no longer right itself. IS Code: angle >= 25 degrees past GZ maximum. Low vanishing angle (< 40 deg) = dangerously unstable vessel.","The permanent list angle","The maximum trim angle"],correct:1,expl:"The angle of vanishing stability is the heel angle at which GZ returns to zero after its positive maximum. Beyond this, the vessel can no longer right itself — it capsizes. For a conventional cargo vessel, the IS Code 2008 requires this angle to be at least 25 degrees past the GZ maximum. A high vanishing angle (> 60-70 deg) indicates a very safe vessel with large stability reserve. The GZ curve and this angle are calculated by stability software for each loading condition."},
  ],
  es: [
    {q:"Que es la flotabilidad y el principio de Arquimedes?",opts:["La estabilidad del buque","Fuerza ascendente = peso del volumen de agua desplazada. El buque flota si empuje = peso total.","El peso del ancla","La curva GZ"],correct:1,expl:"Arquimedes: todo cuerpo sumergido recibe una fuerza ascendente igual al peso del volumen de fluido desplazado."},
    {q:"Cual es la formula del GM y que significan K, G y M?",opts:["GM = KG x KM","GM = KM - KG. K = quilla (fondo), G = centro de gravedad (masas buque), M = metacentro (virtual). GM > 0 = estable.","GM = KG - KM","GM = G / M"],correct:1,expl:"GM = KM - KG es la formula fundamental de la estabilidad inicial. K = quilla = referencia. Si GM > 0: G bajo M = estable."},
    {q:"Que es el brazo adrizante GZ?",opts:["El peso de G","Distancia horizontal entre G y la vertical de B al escorar. Si GZ > 0: par adrizante. Si GZ < 0: par zozobrante.","El calado","La distancia KM"],correct:1,expl:"GZ es el brazo del par adrizante (o zozobrante). La curva GZ da toda la informacion de estabilidad. El umbral reglamentario estandar exige GZ max >= 0,20 m a un angulo >= 30 grados."},
    {q:"Cuales son los criterios minimos del IS Code para la estabilidad inicial?",opts:["Solo GM >= 1 m","GM >= 0,15 m, GZ max >= 0,20 m a >= 30 deg, angulo zozobra >= 25 tras max, areas 0-30 >= 0,055 / 0-40 >= 0,090 m.rad","Solo GZ max >= 0,50 m","GM >= 0,50 m y GZ >= 0,30 m"],correct:1,expl:"IS Code 2008: (1) GM >= 0,15 m. (2) GZ max >= 0,20 m a >= 30. (3) Angulo zozobra >= 25 tras max. (4) Areas 0-30 >= 0,055 / 0-40 >= 0,090 / 30-40 >= 0,030 m.rad."},
    {q:"Como se calcula el efecto de superficie libre?",opts:["GG' = i x densidad liquido / Desplazamiento buque (toneladas)","GG' = GM x seno(escora)","GG' = KM - KG","GG' = longitud x anchura tanque"],correct:0,expl:"Correccion superficie libre: GG' = (i x gamma_liquido) / Desplazamiento. Dividir tanque de anchura b en dos reduce i por 4, GG' por 4."},
    {q:"Cual es la diferencia entre un buque 'rigido' y un buque 'blando'?",opts:["Rigido = mas pesado","Rigido (GM muy positivo) = movimientos bruscos, incomodidad, fatiga estructuras. Blando (GM bajo) = escora lenta y prolongada, riesgo si evento adicional","Rigido = inestable","No hay diferencia practica"],correct:1,expl:"Rigido (GM > 2m): se adiza muy rapido con movimientos violentos. Riesgos: incomodidad tripulacion, fatiga estructuras, desplazamiento carga. Blando (GM bajo): escora lenta. Si evento adicional, puede no adrizar. Ideal: GM alrededor de 0,5 - 1,5m."},
    {q:"Que es el francobordo y por que esta relacionado con la estabilidad?",opts:["La altura de los mastiles","Distancia linea de flotacion - cubierta principal. Determina la reserva de flotabilidad. Gran francobordo = angulo de zozobra alto. Plimsoll fija el minimo legal por zona.","El color del casco","La distancia entre cubiertas"],correct:1,expl:"El francobordo determina la RESERVA DE FLOTABILIDAD. La linea de carga Plimsoll fija el minimo legal por zona. Francobordo insuficiente = angulo de zozobra reducido = peligro."},
    {q:"Que es la experiencia de estabilidad (inclining experiment)?",opts:["Una prueba de velocidad","Ensayo para determinar experimentalmente KG y GM. Masa p desplazada distancia d -> GM = (p x d) / (Desplazamiento x tan theta). Obligatorio en la construccion.","Una prueba de lastre","Un ejercicio de emergencia"],correct:1,expl:"La experiencia de estabilidad es obligatoria en la construccion y tras modificaciones mayores. Metodo: desplazar masa p distancia d -> medir escora theta -> GM = (p x d) / (Desplazamiento x tan theta)."},
    {q:"Cual es la diferencia entre estabilidad inicial y estabilidad a gran inclinacion?",opts:["Sin diferencia","Inicial (< 10-15 deg): caracterizada por GM solo, GZ = GM x sin theta. Gran inclinacion (> 15 deg): se necesita curva GZ completa, el metacentro se desplaza.","Gran inclinacion = cuando GM es alto","La estabilidad inicial es mas importante"],correct:1,expl:"Estabilidad inicial (< 10-15 deg): descrita por GM. A grandes inclinaciones, el metacentro se desplaza y se necesita la curva GZ completa. Un buque puede tener buen GM inicial y zozobrar a 35 grados."},
    {q:"Que es el lastre y cual es su papel en la estabilidad?",opts:["La carga de pago","Agua de mar bombeada a tanques dedicados para ajustar el calado, la asiento y la estabilidad. Llenar tanques bajos -> baja G -> aumenta GM. MARPOL regula las aguas de lastre.","El combustible","El agua de lavado de cubiertas"],correct:1,expl:"El lastre es agua de mar bombeada a tanques dedicados para mejorar la estabilidad. Llenar tanques bajos -> baja G -> aumenta GM. MARPOL regula las aguas de lastre (especies invasoras)."},
    {q:"Que es el asiento y como afecta a la estabilidad?",opts:["La velocidad del buque","Calado popa - calado proa. Asiento positivo (popa mas hundida) = normal y favorece la propulsion. Asiento excesivo cambia KM y puede reducir la estabilidad.","La altura de la carga","El peso total del buque"],correct:1,expl:"El asiento = calado popa - calado proa. Asiento positivo es normal y mejora la eficiencia del helice y timon. Asiento excesivo puede reducir el angulo de zozobra. Respetar siempre los limites del plan de carga."},
    {q:"Que es la estabilidad dinamica y como se mide?",opts:["El GM en el momento de la botadura","Energia total de adrizamiento = area bajo curva GZ de 0 al angulo de zozobra (m.rad). Representa la capacidad de resistir perturbaciones dinamicas.","La velocidad de rotacion del buque","La altura de la ola"],correct:1,expl:"La estabilidad dinamica es la energia total disponible para el adrizamiento = area bajo la curva GZ. IS Code: areas 0-30 >= 0,055 / 0-40 >= 0,090 / 30-40 >= 0,030 m.rad."},
    {q:"Como reducen los mamparos longitudinales el efecto de superficie libre?",opts:["Aumentan el volumen de los tanques","Dividir tanque anchura b en 2 x (b/2): momento de inercia i pasa de l b^3/12 a l b^3/48. Resultado: i dividido entre 4 -> GG' dividido entre 4.","Reducen el peso del liquido","Aumentan el KM"],correct:1,expl:"Formula clave: i = (longitud x anchura^3) / 12. Dividir anchura b en 2: nueva i total = l x b^3 / 48. Cuatro veces menos. Por eso los petroleros tienen tanques multiples estrechos."},
    {q:"Que es el angulo de zozobra y cual es el criterio del IS Code?",opts:["El angulo maximo autorizado de navegacion","Angulo mas alla del cual GZ vuelve a cero y el buque no puede adrizar. IS Code: angulo >= 25 grados tras el maximo GZ. Angulo de zozobra bajo (< 40 grados) = buque inestable.","El angulo de escora permanente","El angulo de asiento maximo"],correct:1,expl:"El angulo de zozobra es el angulo al que GZ vuelve a cero. Mas alla, el buque no puede adrizar. IS Code: angulo >= 25 grados tras el maximo GZ. Angulo alto (> 60-70 grados) = buque muy seguro."},
  ],
  pt: [
    {q:"O que e a flutuabilidade e o principio de Arquimedes?",opts:["A estabilidade do navio","Forca ascendente = peso do volume de agua deslocada. O navio flutua se empuxo = peso total.","O peso da ancora","A curva GZ"],correct:1,expl:"Arquimedes: todo corpo submerso recebe uma forca ascendente igual ao peso do volume de fluido deslocado."},
    {q:"Qual e a formula do GM e o que significam K, G e M?",opts:["GM = KG x KM","GM = KM - KG. K = quilha (fundo), G = centro de gravidade (massas navio), M = metacentro (virtual). GM > 0 = estavel.","GM = KG - KM","GM = G / M"],correct:1,expl:"GM = KM - KG e a formula fundamental da estabilidade inicial. K = quilha = referencia. Se GM > 0: G abaixo de M = estavel."},
    {q:"O que e o braco de endireitamento GZ?",opts:["O peso de G","Distancia horizontal entre G e a vertical de B ao adornar. Se GZ > 0: binas de endireitamento. Se GZ < 0: binas de tombamento.","O calado","A distancia KM"],correct:1,expl:"GZ e o braco das binas de endireitamento (ou tombamento). A curva GZ da toda a informacao de estabilidade. O limiar regulamentar padrao exige GZ max >= 0,20 m a angulo >= 30 graus."},
    {q:"Quais sao os criterios minimos do IS Code para a estabilidade inicial?",opts:["Apenas GM >= 1 m","GM >= 0,15 m, GZ max >= 0,20 m a >= 30 graus, angulo tombamento >= 25 apos max, areas 0-30 >= 0,055 / 0-40 >= 0,090 m.rad","Apenas GZ max >= 0,50 m","GM >= 0,50 m e GZ >= 0,30 m"],correct:1,expl:"IS Code 2008: (1) GM >= 0,15 m. (2) GZ max >= 0,20 m a >= 30. (3) Angulo tombamento >= 25 apos max. (4) Areas 0-30 >= 0,055 / 0-40 >= 0,090 / 30-40 >= 0,030 m.rad."},
    {q:"Como se calcula o efeito de superficie livre?",opts:["GG' = i x densidade liquido / Deslocamento navio (toneladas)","GG' = GM x seno(banda)","GG' = KM - KG","GG' = comprimento x largura tanque"],correct:0,expl:"Correcao superficie livre: GG' = (i x gamma_liquido) / Deslocamento. Dividir tanque de largura b em dois reduz i por 4, GG' por 4."},
    {q:"Qual e a diferenca entre um navio 'rigido' e um navio 'mole'?",opts:["Rigido = mais pesado","Rigido (GM muito positivo) = movimentos bruscos, desconforto, fadiga estruturas. Mole (GM baixo) = banda lenta e prolongada, risco se evento adicional","Rigido = instavel","Nao ha diferenca pratica"],correct:1,expl:"Rigido (GM > 2m): endireita-se muito rapido com movimentos violentos. Mole (GM baixo): banda lenta. Se evento adicional, pode nao endireitar. Ideal: GM cerca de 0,5 - 1,5m."},
    {q:"O que e o bordo livre e por que esta ligado a estabilidade?",opts:["A altura dos mastros","Distancia linha de agua - convés principal. Determina a reserva de flutuabilidade. Grande bordo livre = angulo de tombamento alto. Plimsoll fixa o minimo legal por zona.","A cor do casco","A distancia entre conveses"],correct:1,expl:"O bordo livre determina a RESERVA DE FLUTUABILIDADE. A linha de carga Plimsoll fixa o minimo legal por zona. Bordo livre insuficiente = angulo de tombamento reduzido = perigo."},
    {q:"O que e a experiencia de estabilidade (inclining experiment)?",opts:["Um teste de velocidade","Ensaio para determinar experimentalmente KG e GM. Massa p deslocada distancia d -> GM = (p x d) / (Deslocamento x tan theta). Obrigatorio na construcao.","Um teste de lastro","Um exercicio de emergencia"],correct:1,expl:"A experiencia de estabilidade e obrigatoria na construcao e apos modificacoes maiores. Metodo: deslocar massa p distancia d -> medir banda theta -> GM = (p x d) / (Deslocamento x tan theta)."},
    {q:"Qual e a diferenca entre estabilidade inicial e estabilidade a grande inclinacao?",opts:["Sem diferenca","Inicial (< 10-15 graus): caracterizada por GM so, GZ = GM x sin theta. Grande inclinacao (> 15 graus): precisa curva GZ completa, o metacentro deslocas-se.","Grande inclinacao = quando GM e alto","A estabilidade inicial e mais importante"],correct:1,expl:"Estabilidade inicial (< 10-15 graus): descrita por GM. A grandes inclinacoes, o metacentro desloca-se e precisa-se da curva GZ completa. Um navio pode ter bom GM inicial e tombar a 35 graus."},
    {q:"O que e o lastro e qual o seu papel na estabilidade?",opts:["A carga pagante","Agua do mar bombeada para tanques dedicados para ajustar o calado, o assentamento e a estabilidade. Encher tanques baixos -> baixa G -> aumenta GM. MARPOL regula as aguas de lastro.","O combustivel","A agua de lavagem dos conveses"],correct:1,expl:"O lastro e agua do mar bombeada para tanques dedicados para melhorar a estabilidade. Encher tanques baixos -> baixa G -> aumenta GM. MARPOL regula as aguas de lastro (especies invasoras)"},
    {q:"O que e o assentamento e como afeta a estabilidade?",opts:["A velocidade do navio","Calado popa - calado proa. Assentamento positivo (popa mais funda) = normal e favorece a propulsao. Assentamento excessivo altera KM e pode reduzir a estabilidade.","A altura da carga","O peso total do navio"],correct:1,expl:"O assentamento = calado popa - calado proa. Assentamento positivo e normal e melhora a eficiencia da helice e do leme. Assentamento excessivo pode reduzir o angulo de tombamento. Respeitar sempre os limites do plano de carga."},
    {q:"O que e a estabilidade dinamica e como se mede?",opts:["O GM no momento do lancamento","Energia total de endireitamento = area sob curva GZ de 0 ao angulo de tombamento (m.rad). Representa a capacidade de resistir perturbacoes dinamicas.","A velocidade de rotacao do navio","A altura da onda"],correct:1,expl:"A estabilidade dinamica e a energia total disponivel para o endireitamento = area sob a curva GZ. IS Code: areas 0-30 >= 0,055 / 0-40 >= 0,090 / 30-40 >= 0,030 m.rad."},
    {q:"Como as anteparas longitudinais reduzem o efeito de superficie livre?",opts:["Aumentam o volume dos tanques","Dividir tanque largura b em 2 x (b/2): momento de inercia i passa de l b^3/12 a l b^3/48. Resultado: i dividido por 4 -> GG' dividido por 4.","Reduzem o peso do liquido","Aumentam o KM"],correct:1,expl:"Formula chave: i = (comprimento x largura^3) / 12. Dividir largura b em 2: nova i total = l x b^3 / 48. Quatro vezes menos. Por isso os petroleiros tem tanques multiplos estreitos."},
    {q:"O que e o angulo de tombamento e qual e o criterio do IS Code?",opts:["O angulo maximo autorizado de navegacao","Angulo alem do qual GZ volta a zero e o navio nao pode mais endireitar. IS Code: angulo >= 25 graus apos o maximo GZ. Angulo de tombamento baixo (< 40 graus) = navio instavel.","O angulo de banda permanente","O angulo de assentamento maximo"],correct:1,expl:"O angulo de tombamento e o angulo em que GZ volta a zero. Alem disto, o navio nao pode mais endireitar. IS Code: angulo >= 25 graus apos o maximo GZ. Angulo alto (> 60-70 graus) = navio muito seguro."},
  ],
};
...
export const QUIZ={
  fr:[
    {q:"Un navire a KG = 7,8 m et KM = 8,3 m. Quelle est la valeur de GM et l'etat de stabilite ?",
      ...
      correct:0,expl:"GM = KM - KG = 8,3 - 7,8 = 0,5 m. GM positif = G sous M = STABLE. Valeur de 0,5 m : navire stable mais relativement mou. Seuil reglementaire standard = 0,15 m — critere satisfait. Un GM entre 0,5 et 1,5 m est generalement considere comme acceptable pour un navire de charge."},
    {q:"Quel est l'effet d'un chargement en hauteur (pont superieur) sur la stabilite ?",
      ...
      correct:1,expl:"Charger en hauteur (pont superieur, conteneurs hauts, superstructures) eleve le centre de gravite G. Comme GM = KM - KG, si KG augmente et KM reste approximativement constant, GM diminue. Le navire devient plus mou (gite plus lente) et peut devenir instable si G depasse M. Solution : augmenter le ballast bas pour abaisser G."},
  ...
  en:[
    {q:"A vessel has KG = 7.8 m and KM = 8.3 m. What is GM and the stability state?",
      ...
      correct:0,expl:"GM = KM - KG = 8.3 - 7.8 = 0.5 m. Positive GM = G below M = STABLE. Value of 0.5 m: stable but relatively tender. Standard regulatory threshold = 0.15 m — criterion satisfied. A GM between 0.5 and 1.5 m is generally considered acceptable for a cargo vessel."},
    {q:"What is the effect of high loading (upper deck) on stability?",
      ...
      correct:1,expl:"Loading high (upper deck, tall containers, superstructures) raises the centre of gravity G. Since GM = KM - KG, if KG increases and KM stays approximately constant, GM falls. The vessel becomes more tender (slower roll) and can become unstable if G exceeds M. Solution: increase low ballast to lower G."},
    {q:"What is the free surface effect and how to reduce it?",
      ...
      correct:1,expl:"Free surface effect: when a tank is partially filled, liquid shifts when the vessel heels, creating a destabilising moment. Equivalent to a virtual rise in G of GG' = (i x gamma_liquid) / Delta. Corrected GM = initial GM - GG'. To reduce: (1) fill or empty tanks completely. (2) fit longitudinal divisions (divides i and thus GG' by 4 with central division)."},
    {q:"What is the minimum GM required by the IS Code?",
      opts:["0.50 m","0.15 m","1.00 m","0.30 m"],
      correct:1,expl:"The IS Code 2008 (MSC.267(85)) requires a minimum initial GM of 0.15 m in all planned loading conditions. This criterion must be met accounting for free surface corrections. Additional criteria: max GZ >= 0.20 m at >= 30 deg, vanishing angle >= 25 deg past GZ max, and GZ curve area requirements."},
  ...
  es:[
    {q:"Un buque tiene KG = 7,8 m y KM = 8,3 m. ?Cual es el GM y el estado de estabilidad?",
      ...
      correct:0,expl:"GM = KM - KG = 8,3 - 7,8 = 0,5 m. GM positivo = G bajo M = ESTABLE. Valor de 0,5 m: estable pero relativamente blando. Umbral reglamentario estandar = 0,15 m — criterio satisfecho. Un GM entre 0,5 y 1,5 m es generalmente aceptable para un buque de carga."},
    {q:"?Cual es el efecto de cargar en altura (cubierta superior) sobre la estabilidad?",
      ...
      correct:1,expl:"Cargar en altura eleva el centro de gravedad G. Como GM = KM - KG, si KG aumenta y KM permanece constante, GM disminuye. El buque se vuelve mas blando y puede volverse inestable si G supera M. Solucion: aumentar el lastre bajo para bajar G."},
    {q:"?Que es el efecto de superficie libre y como reducirlo?",
      ...
      correct:1,expl:"Efecto superficie libre: cuando un tanque esta parcialmente lleno, el liquido se desplaza al escorar, creando un momento desestabilizador. Equivale a una elevacion virtual de G de GG' = (i x gamma_liquido) / Desplazamiento. GM corregido = GM inicial - GG'. Para reducir: (1) llenar o vaciar completamente. (2) mamparos longitudinales (divide i y GG' entre 4 con mamparo central)."},
    {q:"?Cual es el GM minimo exigido por el IS Code?",
      opts:["0,50 m","0,15 m","1,00 m","0,30 m"],
      correct:1,expl:"El IS Code 2008 (MSC.267(85)) exige un GM inicial minimo de 0,15 m en todas las condiciones de carga previstas. Criterios adicionales: GZ max >= 0,20 m a >= 30 grados, angulo de zozobra >= 25 grados tras max GZ, y requisitos de areas bajo la curva GZ."},
  ...
  pt:[
    {q:"Um navio tem KG = 7,8 m e KM = 8,3 m. Qual e o GM e o estado de estabilidade?",
      ...
      correct:0,expl:"GM = KM - KG = 8,3 - 7,8 = 0,5 m. GM positivo = G abaixo de M = ESTAVEL. Valor de 0,5 m: estavel mas relativamente mole. Limiar regulamentar padrao = 0,15 m — criterio satisfeito. Um GM entre 0,5 e 1,5 m e geralmente aceitavel para um navio de carga."},
    {q:"Qual e o efeito de carregar em altura (convés superior) na estabilidade?",
      ...
      correct:1,expl:"Carregar em altura eleva o centro de gravidade G. Como GM = KM - KG, se KG aumenta e KM permanece constante, GM desce. O navio torna-se mais mole e pode tornar-se instavel se G ultrapassar M. Solucao: aumentar o lastro baixo para baixar G."},
    {q:"O que e o efeito de superficie livre e como reduzi-lo?",
      ...
      correct:1,expl:"Efeito superficie livre: quando um tanque esta parcialmente cheio, o liquido desloca-se ao adornar, criando um momento desestabilizador. Equivale a uma elevacao virtual de G de GG' = (i x gamma_liquido) / Deslocamento. GM corrigido = GM inicial - GG'. Para reduzir: (1) encher ou esvaziar completamente. (2) anteparas longitudinais (divide i e GG' por 4 com antepara central)."},
    {q:"Qual e o GM minimo exigido pelo IS Code?",
      opts:["0,50 m","0,15 m","1,00 m","0,30 m"],
      correct:1,expl:"O IS Code 2008 (MSC.267(85)) exige um GM inicial minimo de 0,15 m em todas as condicoes de carga previstas. Criterios adicionais: GZ max >= 0,20 m a >= 30 graus, angulo de tombamento >= 25 graus apos max GZ, e requisitos de areas sob a curva GZ."},
...
const getContent=(lang)=>{
  const d={
    fr:{
      ...
      p4:"PARTIE 4 — FACTEURS ET CADRE REGLEMENTAIRE",s4t:"Facteurs + IS Code + Franc-bord Plimsoll",
    ...
    en:{
      ...
      p4:"PART 4 — FACTORS AND REGULATORY FRAMEWORK",s4t:"Factors + IS Code + Plimsoll Freeboard",
      ...
      sumP:["GM = KM - KG — G below M (GM > 0) = stable — G above M (GM < 0) = unstable","IS Code 2008: min GM = 0.15 m — max GZ >= 0.20 m at >= 30 — vanishing angle >= 25 past max","B moves laterally when vessel heels — creates righting GZ lever","Free surface: GG' = i x densL / D — divide tank by 2 = GG' divided by 4","High loading -> G rises -> GM falls -> tender or unstable","Stiff vessel (GM > 2m): sudden movements — tender (GM < 0.15m): slow heel","Inclining experiment: GM = (p x d) / (Displacement x tan theta)","Plimsoll freeboard: reserve of buoyancy — legal minimum by zone","Vanishing angle: GZ = 0 after its maximum — IS Code: >= 25 deg past max","Herald of Free Enterprise 1987: compliant GM, open doors, flooding = 193 deaths"],
    ...
    es:{
      ...
      p4:"PARTE 4 — FACTORES Y MARCO REGLAMENTARIO",s4t:"Factores + IS Code + Francobordo Plimsoll",
      ...
      sumP:["GM = KM - KG — G bajo M = estable — G sobre M = inestable","IS Code 2008: GM min = 0,15 m — GZ max >= 0,20 m — angulo zozobra >= 25 tras max","B se desplaza al escorar — crea el brazo GZ adrizante","Superficie libre: GG' = i x densL / D — dividir tanque = GG' dividido entre 4","Carga alta -> G sube -> GM baja -> blando o inestable","Rigido (GM > 2m): movimientos bruscos — blando (GM < 0,15m): escora lenta","Experiencia estabilidad: GM = (p x d) / (Desplazamiento x tan theta)","Francobordo Plimsoll: reserva flotabilidad — minimo legal por zona","Angulo zozobra: GZ = 0 tras su maximo — IS Code: >= 25 grados","Herald of Free Enterprise 1987: GM conforme, portas abiertas, inundacion = 193 muertos"],
    ...
    pt:{
      ...
      p4:"PARTE 4 — FATORES E QUADRO REGULATORIO",s4t:"Fatores + IS Code + Bordo livre Plimsoll",
      ...
      sumP:["GM = KM - KG — G abaixo de M = estavel — G acima de M = instavel","IS Code 2008: GM min = 0,15 m — GZ max >= 0,20 m — angulo tombamento >= 25 apos max","B desloca-se ao adornar — cria o braco GZ de endireitamento","Superficie livre: GG' = i x densL / D — dividir tanque = GG' dividido por 4","Carga alta -> G sobe -> GM desce -> mole ou instavel","Rigido (GM > 2m): movimentos bruscos — mole (GM < 0,15m): banda lenta","Experiencia estabilidade: GM = (p x d) / (Deslocamento x tan theta)","Bordo livre Plimsoll: reserva flutuabilidade — minimo legal por zona","Angulo tombamento: GZ = 0 apos o seu maximo — IS Code: >= 25 graus","Herald of Free Enterprise 1987: GM conforme, portas abertas, inundacao = 193 mortos"],
      learnedP:["G, B, M, K — formula GM = KM - KG","Curva GZ e criterios IS Code 2008","5 estados de estabilidade","Fatores: superficie livre, carga alta, gelo, inundacao","Bordo livre Plimsoll e experiencia de estabilidade"],
    },
  };
  return d[lang]||d.fr;
};
          ...
          {phase==="content"&&<>
            ...
            {[
              ...
              {icon:"⚠️",p:lc.p4,s:lc.s4t,content:lc.s4,color:C.danger,
                ...
                svgLabel:lbl("FACTEURS + IS CODE + FRANC-BORD","FACTORS + IS CODE + FREEBOARD","FACTORES + IS CODE + FRANCOBORDO","FATORES + IS CODE + BORDO LIVRE")},
            ].map((sec,i)=>(
              <div key={i}>
