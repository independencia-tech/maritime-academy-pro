...
export const BANK_ALL={
  ...
  en:[
    {q:"CC → TC conversion formula. What does each term represent?",opts:["TC = CC - δ - d","TC = CC + δ + d (δ=deviation, d=declination, East=+, West=-)","TC = CC × (δ + d)","TC = CC / δ"],correct:1,expl:"TC = CC + δ + d. δ = deviation (+ if East, - if West). d = declination (+ if East, - if West). East = Add. West = Subtract. For TC→CC, reverse the operations."},
    {q:"A compass deviation varies with:",opts:["Vessel's geographic position","Vessel heading (metallic masses reorient with direction)","Season and weather","Speed only"],correct:1,expl:"Deviation varies with vessel HEADING because onboard metallic masses and electric fields reorient differently depending on the direction. That's why a deviation table provides δ for each compass heading."},
    {q:"Magnetic declination off Cameroon (Gulf of Guinea) is approximately:",opts:["15°E (very strong)","8°W (strong)","0° to 1°W (nearly zero)","45°E (extreme)"],correct:2,expl:"Off Cameroon and the Gulf of Guinea, magnetic declination is nearly zero (0° to 1°W). A favorable area for magnetic compass navigation since correction is minimal."},
    {q:"CC=135° · δ=-2°W · d=+4°E — Calculate TC.",opts:["129°","133°","137°","141°"],correct:2,expl:"TC = CC + δ + d = 135 + (-2) + 4 = 137°. W=negative (-2°), E=positive (+4°). Result: 135 - 2 + 4 = 137°."},
    {q:"What is the difference between 'Set' and 'Drift' of a current?",opts:["Set=speed · Drift=direction","Set=direction the current flows TO · Drift=current speed","Set=surface current · Drift=bottom current","Identical"],correct:1,expl:"SET = direction the current GOES toward (e.g. Set 045° = current flowing NE). DRIFT = current speed in knots (e.g. Drift 1.5 kn)."},
    {q:"A vessel heads 000°N at 10 kn. Current Set 090°E, Drift 3 kn. Which way does it drift?",opts:["Stays on 000°","Eastward — track between 000° and 090°","Westward","Speeds up only"],correct:1,expl:"The current pushes east (090°) while the vessel heads north. The resulting track over ground will be between 000° and 090° — slightly east of north."},
    {q:"Wind drift (leeway) depends on:",opts:["Hull color","Wind force, superstructure area, vessel speed, vessel type","Under-keel clearance","Number of crew members"],correct:1,expl:"Leeway = wind drift. Depends on: wind force, superstructure area (tall vessels = more drift), vessel speed (faster = less drift), vessel type (container ships very sensitive). Estimate: 1° to 15°."},
    {q:"TC=270° · d=-5°W · δ=-3°W — Calculate CC.",opts:["262°","268°","278°","284°"],correct:2,expl:"MC = TC - d = 270-(-5) = 275°. CC = MC - δ = 275-(-3) = 278°. Check: 278+(-3)+(-5) = 270°=TC ✓."},
    {q:"The Benguela Current (West Africa):",opts:["Flows southward along the coasts","Flows northward — affects vessels off Cameroon","Warm current toward West Africa","Flows east"],correct:1,expl:"The Benguela Current flows northward along the West African coast (Angola→Cameroon→Nigeria). Cold current from the south. Drift ~0.5 to 1 knot. Creates a northward set for vessels in the region."},
    {q:"What is the velocity triangle?",opts:["Port signaling","Vector construction: course vector + current vector = track over ground","Tide calculation","3-bearing fix system"],correct:1,expl:"Velocity triangle = vector construction: (1) Draw course+speed vector. (2) Add current vector (Set+Drift). (3) Resultant = track over ground + speed over ground. Used to anticipate drift and correct heading."},
    {q:"How long does a gyrocompass need to stabilize?",opts:["30 seconds","4 to 6 hours","24 hours","Instant"],correct:1,expl:"The gyrocompass needs 4 to 6 hours to settle on True North. Started before departure. At polar latitudes (>75°), it becomes less reliable."},
    {q:"GPS Course Over Ground (COG) represents:",opts:["Compass heading shown on the compass","Actual direction of travel including current and wind drift","True course without correction","Heading to the next buoy"],correct:1,expl:"COG (Course Over Ground) = actual direction of movement over ground. Includes current and wind effects. COG ≠ true course when current/wind is present."},
    {q:"CC=000° · δ=0° · d=+10°E — Calculate TC.",opts:["350°","000°","010°","020°"],correct:2,expl:"TC = CC + δ + d = 000 + 0 + 10 = 010°. Only the 10°E declination applies. Compass course 000° = true course 010° because of the 10°E declination."},
    {q:"To compensate for current drift, you must:",opts:["Increase speed only","Adjust course AGAINST the current","Reduce speed","Do nothing — GPS auto-corrects"],correct:1,expl:"To compensate for current drift, aim slightly INTO the current. E.g. current going east + want to go north → steer slightly west of north (e.g. 355° instead of 000°)."},
    {q:"The compass rose divides the horizon into how many parts?",opts:["8 parts","16 parts","32 wind points","360 equal degrees"],correct:2,expl:"Traditional rose = 32 wind points (N, NNE, NE, ENE, E, ESE, SE, SSE, S, SSW, SW, WSW, W, WNW, NW, NNW + subdivisions). Modern navigation: degrees 0°-360°. N=000°, E=090°, S=180°, W=270°."},
  ],
  es:[
    {q:"Fórmula de conversión RC → RV. ¿Qué representa cada término?",opts:["RV = RC - δ - d","RV = RC + δ + d (δ=desvío, d=declinación, Este=+, Oeste=-)","RV = RC × (δ + d)","RV = RC / δ"],correct:1,expl:"RV = RC + δ + d. δ = desvío (+ si Este, - si Oeste). d = declinación (+ si Este, - si Oeste). Este = Sumar. Oeste = Restar. Para RV→RC se invierten las operaciones."},
    {q:"El desvío de un compás varía según:",opts:["La posición geográfica del buque","El rumbo del buque (las masas metálicas se reorientan)","La estación y el clima","Solo la velocidad"],correct:1,expl:"El desvío varía según el RUMBO del buque porque las masas metálicas y campos eléctricos a bordo se reorientan según la dirección. Por eso se usa una tabla de desvío con δ para cada rumbo de compás."},
    {q:"La declinación frente a Camerún (Golfo de Guinea) es aproximadamente:",opts:["15°E (muy fuerte)","8°O (fuerte)","0° a 1°O (casi nula)","45°E (extrema)"],correct:2,expl:"Frente a Camerún y el Golfo de Guinea, la declinación magnética es casi nula (0° a 1°O). Zona favorable para la navegación con compás magnético, la corrección es mínima."},
    {q:"RC=135° · δ=-2°O · d=+4°E — Calcula RV.",opts:["129°","133°","137°","141°"],correct:2,expl:"RV = RC + δ + d = 135 + (-2) + 4 = 137°. O=negativo (-2°), E=positivo (+4°). Cálculo: 135 - 2 + 4 = 137°."},
    {q:"¿Cuál es la diferencia entre 'Set' y 'Drift' de una corriente?",opts:["Set=velocidad · Drift=dirección","Set=dirección hacia donde va la corriente · Drift=velocidad","Set=corriente superficial · Drift=corriente de fondo","Idénticos"],correct:1,expl:"SET = dirección hacia la que VA la corriente (ej: Set 045° = corriente hacia NE). DRIFT = velocidad de la corriente en nudos (ej: Drift 1,5 kn)."},
    {q:"Un buque navega rumbo 000°N a 10 kn. Corriente Set 090°E, Drift 3 kn. ¿Hacia dónde deriva?",opts:["Mantiene rumbo 000°","Hacia el Este — derrota entre 000° y 090°","Hacia el Oeste","Solo acelera"],correct:1,expl:"La corriente empuja hacia el Este (090°) mientras el buque va al Norte. La derrota resultante estará entre 000° y 090° — ligeramente al Este del Norte."},
    {q:"La deriva por el viento (abatimiento) depende de:",opts:["El color del casco","Fuerza del viento, superficie de la superestructura, velocidad y tipo de buque","Profundidad bajo la quilla","Número de tripulantes"],correct:1,expl:"Abatimiento = deriva por viento. Depende de: fuerza del viento, superficie de superestructura (buques altos = más deriva), velocidad (más rápido = menos deriva), tipo de buque (portacontenedores muy sensible). Estimación: 1° a 15°."},
    {q:"RV=270° · d=-5°O · δ=-3°O — Calcula RC.",opts:["262°","268°","278°","284°"],correct:2,expl:"RM = RV - d = 270-(-5) = 275°. RC = RM - δ = 275-(-3) = 278°. Comprobación: 278+(-3)+(-5) = 270°=RV ✓."},
    {q:"La corriente de Benguela (África occidental):",opts:["Va hacia el Sur por las costas","Va hacia el Norte — afecta buques frente a Camerún","Corriente cálida hacia África del Oeste","Va hacia el Este"],correct:1,expl:"La corriente de Benguela sube hacia el Norte por las costas africanas occidentales (Angola→Camerún→Nigeria). Corriente fría procedente del sur. Drift ~0,5 a 1 nudo."},
    {q:"¿Qué es el triángulo de velocidades?",opts:["Señalización portuaria","Construcción vectorial: vector rumbo + vector corriente = vector derrota","Cálculo de mareas","Sistema de demoras a 3 puntos"],correct:1,expl:"Triángulo de velocidades = construcción vectorial: (1) Trazar vector rumbo+velocidad. (2) Sumar vector corriente (Set+Drift). (3) Resultante = derrota y velocidad sobre el fondo."},
    {q:"¿Cuánto tiempo necesita el girocompás para estabilizarse?",opts:["30 segundos","4 a 6 horas","24 horas","Instantáneo"],correct:1,expl:"El girocompás necesita 4 a 6 horas para estabilizarse en el Norte Verdadero. Se arranca antes de zarpar. En latitudes polares (>75°) pierde fiabilidad."},
    {q:"El rumbo sobre el fondo (COG) del GPS representa:",opts:["El rumbo de compás","La dirección real del desplazamiento incluyendo deriva por corriente y viento","El rumbo verdadero sin corrección","El rumbo hacia la siguiente boya"],correct:1,expl:"COG (Course Over Ground) = dirección real del desplazamiento respecto al fondo. Incluye corriente y viento. COG ≠ rumbo verdadero si hay corriente/viento."},
    {q:"RC=000° · δ=0° · d=+10°E — Calcula RV.",opts:["350°","000°","010°","020°"],correct:2,expl:"RV = RC + δ + d = 000 + 0 + 10 = 010°. Solo se aplica la declinación de 10°E."},
    {q:"Para compensar la deriva por corriente, hay que:",opts:["Aumentar velocidad","Corregir el rumbo en sentido CONTRARIO a la corriente","Reducir velocidad","Nada — el GPS corrige automáticamente"],correct:1,expl:"Para compensar la deriva, se apunta el rumbo ligeramente CONTRA la corriente. Ej: corriente al Este + se quiere ir al Norte → gobernar ligeramente al Oeste del Norte (ej: 355° en lugar de 000°)."},
    {q:"¿En cuántas partes divide la rosa de los vientos el horizonte?",opts:["8 partes","16 partes","32 cuartas","360 grados iguales"],correct:2,expl:"Rosa tradicional = 32 cuartas. Navegación moderna: grados 0°-360°. N=000°, E=090°, S=180°, O=270°."},
  ],
  pt:[
    {q:"Fórmula de conversão RB → RV. O que representa cada termo?",opts:["RV = RB - δ - d","RV = RB + δ + d (δ=desvio, d=declinação, Este=+, Oeste=-)","RV = RB × (δ + d)","RV = RB / δ"],correct:1,expl:"RV = RB + δ + d. δ = desvio (+ se Este, - se Oeste). d = declinação (+ se Este, - se Oeste). Para RV→RB invertem-se as operações."},
    {q:"O desvio de uma bússola varia conforme:",opts:["A posição geográfica do navio","O rumo do navio (massas metálicas se reorientam)","Estação e meteorologia","Apenas a velocidade"],correct:1,expl:"O desvio varia conforme o RUMO do navio porque as massas metálicas e campos elétricos a bordo se reorientam segundo a direção. Por isso usa-se uma tabela de desvio com δ para cada rumo de bússola."},
    {q:"A declinação ao largo de Camarões (Golfo da Guiné) é aproximadamente:",opts:["15°E (muito forte)","8°O (forte)","0° a 1°O (quase nula)","45°E (extrema)"],correct:2,expl:"Ao largo de Camarões e no Golfo da Guiné, a declinação magnética é quase nula (0° a 1°O). Zona favorável para navegação com bússola magnética."},
    {q:"RB=135° · δ=-2°O · d=+4°E — Calcula RV.",opts:["129°","133°","137°","141°"],correct:2,expl:"RV = RB + δ + d = 135 + (-2) + 4 = 137°. O=negativo, E=positivo. Cálculo: 135 - 2 + 4 = 137°."},
    {q:"Qual é a diferença entre 'Set' e 'Drift' de uma corrente?",opts:["Set=velocidade · Drift=direção","Set=direção para onde vai a corrente · Drift=velocidade","Set=corrente superficial · Drift=corrente de fundo","Idênticos"],correct:1,expl:"SET = direção para onde VAI a corrente. DRIFT = velocidade da corrente em nós."},
    {q:"Um navio segue rumo 000°N a 10 nós. Corrente Set 090°E, Drift 3 nós. Para onde deriva?",opts:["Mantém rumo 000°","Para Este — rumo de fundo entre 000° e 090°","Para Oeste","Apenas acelera"],correct:1,expl:"A corrente empurra para Este (090°) enquanto o navio vai para Norte. O rumo de fundo resultante ficará entre 000° e 090° — ligeiramente a Este do Norte."},
    {q:"A deriva pelo vento (abatimento) depende de:",opts:["A cor do casco","Força do vento, área da superestrutura, velocidade e tipo do navio","Profundidade sob a quilha","Número de tripulantes"],correct:1,expl:"Abatimento depende da força do vento, área da superestrutura, velocidade e tipo de navio. Estimativa: 1° a 15°."},
    {q:"RV=270° · d=-5°O · δ=-3°O — Calcula RB.",opts:["262°","268°","278°","284°"],correct:2,expl:"RM = RV - d = 270-(-5) = 275°. RB = RM - δ = 275-(-3) = 278°. Verificação: 278+(-3)+(-5) = 270° ✓."},
    {q:"A corrente de Benguela (África ocidental):",opts:["Vai para Sul ao longo das costas","Vai para Norte — afeta navios ao largo de Camarões","Corrente quente para África Ocidental","Vai para Este"],correct:1,expl:"A corrente de Benguela sobe para Norte ao longo das costas africanas ocidentais. Corrente fria. Drift ~0,5 a 1 nó."},
    {q:"O que é o triângulo das velocidades?",opts:["Sinalização portuária","Construção vetorial: vetor rumo + vetor corrente = vetor rumo de fundo","Cálculo de marés","Sistema de marcações em 3 pontos"],correct:1,expl:"Triângulo das velocidades = construção vetorial somando rumo+velocidade e corrente (Set+Drift) para obter rumo e velocidade sobre o fundo."},
    {q:"Quanto tempo a girobússola precisa para estabilizar?",opts:["30 segundos","4 a 6 horas","24 horas","Instantâneo"],correct:1,expl:"A girobússola necessita 4 a 6 horas para estabilizar no Norte Verdadeiro. Liga-se antes da partida. Em latitudes polares (>75°) torna-se menos fiável."},
    {q:"O rumo sobre o fundo (COG) dado pelo GPS representa:",opts:["O rumo de bússola","A direção real do deslocamento incluindo deriva por corrente e vento","O rumo verdadeiro sem correção","O rumo para a próxima boia"],correct:1,expl:"COG = direção real do deslocamento em relação ao fundo. Inclui efeitos de corrente e vento."},
    {q:"RB=000° · δ=0° · d=+10°E — Calcula RV.",opts:["350°","000°","010°","020°"],correct:2,expl:"RV = RB + δ + d = 000 + 0 + 10 = 010°. Apenas a declinação de 10°E se aplica."},
    {q:"Para compensar a deriva pela corrente, deve-se:",opts:["Aumentar a velocidade","Corrigir o rumo no sentido OPOSTO à corrente","Reduzir a velocidade","Nada — o GPS corrige automaticamente"],correct:1,expl:"Para compensar a deriva, aponta-se ligeiramente CONTRA a corrente. Ex: corrente para Este e se quer ir a Norte → governar ligeiramente a Oeste do Norte (355° em vez de 000°)."},
    {q:"Em quantas partes a rosa-dos-ventos divide o horizonte?",opts:["8 partes","16 partes","32 quartas","360 graus iguais"],correct:2,expl:"Rosa tradicional = 32 quartas. Navegação moderna: graus 0°-360°. N=000°, E=090°, S=180°, O=270°."},
  ],
};
    ...
    <div>
      ...
      {answered&&<>
        ...
        <button onClick={next} style={{width:"100%",padding:"11px 0",border:"none",borderRadius:12,background:`linear-gradient(135deg,${C.purple},${C.blue})`,fontFamily:"'Cinzel',serif",fontSize:12,fontWeight:700,letterSpacing:2,color:C.white,cursor:"pointer"}}>
          {cur<BANK.length-1?nextLabel:finishLabel}
        </button>
      </>}
    </div>
  );
...
// MAIN COMPONENT
// ══════════════════════════════════════
export default function LessonCompas({ lang="fr", onBack=()=>{}, onComplete=()=>{}, onNext=()=>{}, onQuizScored=()=>{} }) {
  const t = T[lang]||T.fr;
      ...
      <div style={{position:"relative",zIndex:100,background:"rgba(6,14,26,0.97)",backdropFilter:"blur(14px)",borderBottom:`1px solid ${C.border}`}}>
        <div style={{height:54,display:"flex",alignItems:"center",padding:"0 16px",gap:12}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:C.muted}}>{t.lesson} 5/10</div>
          </div>
