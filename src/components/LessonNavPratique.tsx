...
const BANK = [
  ...
  {q:"Quelle est la règle COLREG qui concerne la veille nautique ?",opts:["Rule 2 — Responsabilité générale","Rule 5 — Veille permanente par tous les moyens disponibles","Rule 10 — Dispositifs de séparation du trafic","Rule 16 — Manœuvre du navire manœuvrant"],correct:1,expl:"COLREG Rule 5 : 'Every vessel shall at all times maintain a proper look-out by sight and hearing as well as by all available means appropriate to the prevailing circumstances.' Veille visuelle + auditive + radar + AIS + VHF = obligatoire 24h/24."},
...
function QuestionBank({ lang }) {
  ...
  return (
    <div>
      ...
      <div style={{height:3,background:"rgba(255,255,255,0.08)",borderRadius:3,marginBottom:12,overflow:"hidden"}}><div style={{height:"100%",width:`${(cur/BANK.length)*100}%`,background:`linear-gradient(90deg,${C.purple},${C.gold2})`}}/></div>
      ...
      <button onClick={next} style={{width:"100%",padding:"11px 0",border:"none",borderRadius:12,background:`linear-gradient(135deg,${C.purple},${C.blue})`,fontFamily:"'Cinzel',serif",fontSize:12,fontWeight:700,letterSpacing:2,color:C.white,cursor:"pointer"}}>{cur<BANK.length-1?(lang==="fr"?"SUIVANT →":lang==="en"?"NEXT →":lang==="es"?"SIGUIENTE →":"PRÓXIMO →"):(lang==="fr"?"TERMINER":"FINISH")}</button></>}
    </div>
  );
}
...
// MAIN
// ══════════════════════════════════════
export default function LessonNavPratique({ lang="fr", onBack=()=>{}, onComplete=()=>{} }) {
  const t = T[lang]||T.fr;
