...
function buildBank(lang) {
  ...
  const qs=[
      ...
      {q:lbl("Que signifie N+1 en redondance AMS ?","What does N+1 mean in AMS redundancy?","Que significa N+1 en redundancia AMS?","O que significa N+1 em redundancia AMS?"),
        ...
        correct:1,expl:lbl("N+1 : toujours au moins un systeme de secours operationnel. CPU secondaire prend le relais en < 100ms sans perte de donnees si le CPU principal tombe.","N+1: always at least one backup system operational. Secondary CPU takes over in < 100ms without data loss if primary CPU fails.","N+1: siempre al menos un sistema de respaldo operacional. CPU secundario asume en < 100ms.","N+1: sempre pelo menos um sistema de backup operacional. CPU secundario assume em < 100ms.")},
      ...
      {q:lbl("Frequence de confirmation Dead Man Alarm en mode UMS ?","DMA confirmation frequency in UMS mode?","Frecuencia de confirmacion Dead Man Alarm en modo UMS?","Frequencia de confirmacao Dead Man Alarm no modo UMS?"),
        ...
        correct:1,expl:lbl("DMA exige confirmation de presence toutes les 30 minutes. Sans confirmation : alarme audivisuelle SM. Non acquittee en 30s : transfer passerelle OOW.","DMA requires presence confirmation every 30 minutes. Without confirmation: ER audivisual alarm. Not acked in 30s: bridge OOW transfer.","DMA exige confirmacion presencia cada 30 minutos.","DMA exige confirmacao presenca a cada 30 minutos.")},
      ...
      {q:lbl("Regle SOLAS sur la redondance des systemes d'alarme ?","SOLAS rule on alarm system redundancy?","Regla SOLAS sobre redundancia de sistemas de alarma?","Regra SOLAS sobre redundancia de sistemas de alarme?"),
        ...
        correct:2,expl:lbl("SOLAS exige qu'une defaillance unique ne puisse pas causer la perte de surveillance d'un parametre critique. D'ou le principe N+1 avec CPU redondant et alimentations independantes.","SOLAS requires that a single failure cannot cause loss of monitoring of a critical parameter. Hence N+1 with redundant CPU and independent power supplies.","SOLAS exige que un fallo unico no cause perdida de vigilancia de parametro critico.","SOLAS exige que uma falha unica nao cause perda de monitoramento de parametro critico.")},
      ...
      {q:lbl("Role du VDR par rapport aux alarmes machine ?","Role of VDR regarding engine alarms?","Papel del VDR con respecto a alarmas de maquinas?","Papel do VDR em relacao aos alarmes de maquinas?"),
        ...
        correct:1,expl:lbl("Le VDR (SOLAS V/20) enregistre donnees navire incluant alarmes machine pendant au moins 48h. En cas d'accident, source principale pour reconstituer la chronologie des evenements.","VDR (SOLAS V/20) records vessel data including engine alarms for at least 48h. In accident case, primary source to reconstruct event timeline.","VDR (SOLAS V/20) registra datos buque incluyendo alarmas maquinas durante 48h minimo.","VDR (SOLAS V/20) registra dados navio incluindo alarmes maquinas por 48h minimo.")},
...
function QuestionBank({ lang, onComplete }) {
  ...
  const qs = buildBank(lang);

  const [shuffled]=useState(()=>qs.map(q=>shuffleQuestionOptions(q,"correct")));
  ...
  return (
    <div>
      ...
      {q.opts.map((opt,i)=>{
        ...
        if(answered){
          if(i===q.correct){bg="rgba(0,230,118,0.12)";brd=green;col=green;}
      ...
      {answered && (
        <div style={{padding:"13px",borderRadius:13,
          ...
          border:`1.5px solid ${sel===q.correct?green:red}55`,marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:800,color:sel===q.correct?green:red,marginBottom:5}}>
            {sel===q.correct
...
export const QUIZ={
  fr:[
    {q:"Que signifie AMS et combien de points d'alarme sur un VLCC ?",opts:["Alarm Monitoring System — > 1 000 points","Alarm Master System — 100 points","Automatic Marine System — 500 points","Alert Monitor Station — 50 points"],correct:0,expl:"AMS = Alarm Monitoring System. Un VLCC moderne peut avoir 1 000 a 5 000 points d'alarme : paliers, pressions, temperatures, niveaux, vibrations, detection feu/gaz. Fabricants : Kongsberg, Wartsila NACOS, ABB."},
    {q:"SOLAS II-1/51 en mode UMS : une alarme non acquittee en 30s :",opts:["Arrete automatiquement le moteur","Est transferee automatiquement vers la passerelle (OOW)","Declenche le systeme CO2","Appelle le chef mecanicien uniquement"],correct:1,expl:"SOLAS II-1 Reg. 51 : transfert automatique a la passerelle (OOW) si non acquittee en 30 secondes. L'OOW doit verifier et appeler l'officier mecanicien de quart."},
    {q:"Que detecte l'OMD (Oil Mist Detector) ?",opts:["Qualite de l'huile","Concentration brouillard huile dans carter — risque explosion","Niveau huile reservoir","Fuites huile externes"],correct:1,expl:"L'OMD detecte la concentration de brouillard d'huile dans le carter. Concentration elevee = echauffement anormal des pieces en mouvement = risque explosion carter. Declenche slow-down puis shutdown."},
    {q:"Le MV Scandinavian Star (1990) a cause 158 morts car :",opts:["Le ME etait defaillant","Detecteurs non fonctionnels + alarmes ACK sans verif + equipage non forme","Le telegraph etait bloque","Le carburant etait epuise"],correct:1,expl:"Triple defaillance : detecteurs non fonctionnels, alarmes acquittees sans investigation physique, equipage sans formation sur le systeme specifique apres changement de pavillon. Lecon cle ISM Code."},
    {q:"La maintenance predictive via AMS permet :",opts:["De remplacer toutes les pieces periodiquement","D'analyser les tendances pour intervenir avant la panne — 70% moins de pannes selon DNV","De reduire le nombre d'alarmes","D'eliminer les rondes UMS"],correct:1,expl:"La maintenance predictive analyse les tendances AMS (vibrations, temperatures, pressions) pour detecter les signes avant-coureurs de panne. Reduction de 70% des immobilisations non planifiees selon DNV GL."},
  ],
  en:[
    {q:"What does AMS mean and how many alarm points on a VLCC?",opts:["Alarm Monitoring System — > 1,000 points","Alarm Master System — 100 points","Automatic Marine System — 500 points","Alert Monitor Station — 50 points"],correct:0,expl:"AMS = Alarm Monitoring System. A modern VLCC can have 1,000 to 5,000 alarm points: bearings, pressures, temperatures, levels, vibrations, fire/gas detection. Manufacturers: Kongsberg, Wartsila NACOS, ABB."},
    {q:"SOLAS II-1/51 in UMS mode: alarm not acknowledged in 30s:",opts:["Automatically stops the engine","Is automatically transferred to the bridge (OOW)","Activates CO2 system","Calls chief engineer only"],correct:1,expl:"SOLAS II-1 Reg. 51: automatic transfer to bridge (OOW) if not acknowledged in 30 seconds. OOW must verify and call the duty engineer."},
    {q:"What does the OMD (Oil Mist Detector) detect?",opts:["Oil quality","Crankcase oil mist concentration — explosion risk","Oil reservoir level","External oil leaks"],correct:1,expl:"OMD detects oil mist concentration in crankcase. High concentration = abnormal heating of moving parts = crankcase explosion risk. Triggers slow-down then shutdown."},
    {q:"MV Scandinavian Star (1990) caused 158 deaths because:",opts:["ME was faulty","Non-functional detectors + alarms ACK without verification + untrained crew","Telegraph was stuck","Fuel was exhausted"],correct:1,expl:"Triple failure: non-functional detectors, alarms acknowledged without physical investigation, crew untrained on specific system after flag change. Key ISM Code lesson."},
    {q:"Predictive maintenance via AMS enables:",opts:["Replacing all parts periodically","Analyzing trends to intervene before failure — 70% fewer breakdowns per DNV","Reducing alarm count","Eliminating UMS rounds"],correct:1,expl:"Predictive maintenance analyzes AMS trends (vibrations, temperatures, pressures) to detect early warning signs of failure. 70% reduction in unplanned downtime per DNV GL."},
  ],
  es:[
    {q:"?Que significa AMS y cuantos puntos de alarma en un VLCC?",opts:["Alarm Monitoring System — > 1.000 puntos","Alarm Master System — 100 puntos","Automatic Marine System — 500 puntos","Alert Monitor Station — 50 puntos"],correct:0,expl:"AMS = Alarm Monitoring System. Un VLCC moderno puede tener 1.000 a 5.000 puntos de alarma. Fabricantes: Kongsberg, Wartsila NACOS, ABB."},
    {q:"SOLAS II-1/51 modo UMS: alarma no acusada en 30s:",opts:["Para automaticamente el motor","Se transfiere automaticamente al puente (OOW)","Activa sistema CO2","Llama al jefe de maquinas solo"],correct:1,expl:"SOLAS II-1 Reg.51: transferencia automatica al puente (OOW) si no acusada en 30 segundos."},
    {q:"?Que detecta el OMD (Oil Mist Detector)?",opts:["Calidad del aceite","Concentracion niebla aceite en carter — riesgo explosion","Nivel deposito aceite","Fugas externas"],correct:1,expl:"El OMD detecta la concentracion de niebla de aceite en el carter. Alta concentracion = riesgo explosion. Activa slow-down y shutdown."},
    {q:"El MV Scandinavian Star (1990) — 158 muertos porque:",opts:["ME averiado","Detectores no funcionales + ACK sin verificacion + tripulacion sin formacion","Telegrafo bloqueado","Combustible agotado"],correct:1,expl:"Triple fallo: detectores no funcionales, alarmas acusadas sin investigacion, tripulacion sin formacion tras cambio pabellon."},
    {q:"El mantenimiento predictivo via AMS permite:",opts:["Reemplazar todas las piezas","Analizar tendencias para intervenir antes de averia — 70% menos paradas segun DNV","Reducir alarmas","Eliminar rondas UMS"],correct:1,expl:"El mantenimiento predictivo analiza tendencias AMS para detectar senales precursoras. Reduccion del 70% en paradas no planificadas segun DNV GL."},
  ],
  pt:[
    {q:"O que significa AMS e quantos pontos de alarme num VLCC?",opts:["Alarm Monitoring System — > 1.000 pontos","Alarm Master System — 100 pontos","Automatic Marine System — 500 pontos","Alert Monitor Station — 50 pontos"],correct:0,expl:"AMS = Alarm Monitoring System. Um VLCC moderno pode ter 1.000 a 5.000 pontos de alarme. Fabricantes: Kongsberg, Wartsila NACOS, ABB."},
    {q:"SOLAS II-1/51 modo UMS: alarme nao acusado em 30s:",opts:["Para automaticamente o motor","E transferido automaticamente para a ponte (OOW)","Ativa sistema CO2","Chama chefe de maquinas apenas"],correct:1,expl:"SOLAS II-1 Reg.51: transferencia automatica para a ponte (OOW) se nao acusado em 30 segundos."},
    {q:"O que deteta o OMD (Oil Mist Detector)?",opts:["Qualidade do oleo","Concentracao nevoa oleo no carter — risco explosao","Nivel deposito oleo","Fugas externas"],correct:1,expl:"O OMD deteta a concentracao de nevoa de oleo no carter. Alta concentracao = risco explosao. Aciona slow-down e shutdown."},
    {q:"O MV Scandinavian Star (1990) — 158 mortos porque:",opts:["ME avariado","Detectores nao funcionais + ACK sem verificacao + tripulacao sem formacao","Telegrafo bloqueado","Combustivel esgotado"],correct:1,expl:"Tripla falha: detectores nao funcionais, alarmes acusados sem investigacao, tripulacao sem formacao apos mudanca de bandeira."},
    {q:"A manutencao preditiva via AMS permite:",opts:["Substituir todas as pecas","Analisar tendencias para intervir antes da avaria — 70% menos paragens segundo DNV","Reduzir alarmes","Eliminar rondas UMS"],correct:1,expl:"A manutencao preditiva analisa tendencias AMS para detetar sinais precursores. Reducao de 70% nas paragens nao planeadas segundo DNV GL."},
  ],
};
...
// ══════════════════════════════════════
export default function LessonE7_L4({ lang="fr", onBack=()=>{}, onComplete=()=>{}, onNext=()=>{}, onQuizScored=(score:number,maxScore:number)=>{} }) {
  const t=T[lang]||T.fr;
          ...
          {phase==="content"&&<>
            ...
            <button disabled={!bankDone} onClick={()=>{if(bankDone)setPhase("quiz");}}
              style={{opacity:bankDone?1:0.45,cursor:bankDone?"pointer":"not-allowed",width:"100%",padding:"18px 0",border:"none",borderRadius:18,
                ...
                color:bg0,boxShadow:`0 10px 40px rgba(0,229,255,0.35)`,marginTop:4}}>
              {t.startQuiz}
            </button>
