...
function CauseMatrixSVG({ lang }) {
  ...
  return (
    <div>
      ...
      {sel_&&<div style={{padding:"10px 12px",borderRadius:12,background:"rgba(201,146,42,0.1)",border:`1px solid ${C.gold}44`,fontSize:11,color:C.white}}><span style={{fontWeight:700,color:C.gold2}}>{lang==="fr"?"Cas concerné : ":"Case: "}</span>{sel_.cases}</div>}
    </div>
  );
}
...
export default function LessonSafetyS2_L5({ lang="fr", onBack=()=>{}, onComplete=()=>{}, onNext=()=>{}, onQuizScored=(score:number,maxScore:number)=>{} }) {
  const t = T[lang]||T.fr;
