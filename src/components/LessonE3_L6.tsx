    ...
    <div>
      ...
      {bankIdx!==null&&!bankDone&&(
        <div>
          ...
          <div style={{fontSize:13,color:"#e0e8ff",lineHeight:1.6,fontFamily:"Courier New",marginBottom:12,padding:12,borderRadius:10,background:"rgba(10,22,40,0.8)",border:`1px solid #6dbf8a22`}}>{shuffledBank[bankCur].q}</div>
...
function QuizTab({ lang, onComplete }:{ lang:string; onComplete:(xp:number,score:number,maxScore:number)=>void }) {
  ...
  if(done) return (
    <div style={{padding:"24px 14px"}}>
      ...
      <button onClick={()=>onComplete(xp,score,quiz.length)} style={{width:"100%",padding:"15px 0",border:"none",borderRadius:14,background:"linear-gradient(135deg,#6dbf8a,#c9922a)",fontFamily:"'Cinzel',serif",fontSize:14,fontWeight:700,letterSpacing:2,color:"#060e1a",cursor:"pointer"}}>🔧 {l.finish}</button>
...
export default function LessonE3_L6({ lang="fr", onBack, onComplete, onQuizScored }:{ lang?:string; onBack:()=>void; onComplete?:(xp?:number)=>void; onQuizScored?:(score:number,maxScore:number)=>void; }) {
  const t=T[lang]||T.fr;
  ...
  const lessonOf=lang==="fr"?"Leçon 6/6":lang==="en"?"Lesson 6/6":lang==="es"?"Lección 6/6":"Lição 6/6";
  const badgeText=lang==="fr"?`🔧 ${moduleFull} · Leçon 6/6 · ⭐ Premium · 200 XP`:lang==="en"?`🔧 ${moduleFull} · Lesson 6/6 · ⭐ Premium · 200 XP`:lang==="es"?`🔧 ${moduleFull} · Lección 6/6 · ⭐ Premium · 200 XP`:`🔧 ${moduleFull} · Lição 6/6 · ⭐ Premium · 200 XP`;
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1f3c,#060e1a)",color:"#f0f4ff",fontFamily:"'Nunito',sans-serif"}}>
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(6,14,26,0.97)",backdropFilter:"blur(14px)",borderBottom:"1px solid rgba(109,191,138,0.22)"}}>
        <div style={{minHeight:54,display:"flex",alignItems:"center",padding:"8px 16px",gap:12}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:10,color:"#6dbf8a",letterSpacing:1,fontFamily:"'Cinzel',serif"}}>🔧 {moduleFull}</div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
            ...
            <div style={{fontSize:11,color:"#6dbf8a",fontFamily:"'Cinzel',serif"}}>{progress}%</div>
          </div>
        </div>
        <div style={{height:3,background:"rgba(255,255,255,0.07)",overflow:"hidden"}}>
          <div style={{height:"100%",width:`${progress}%`,background:"linear-gradient(90deg,#6dbf8a,#c9922a)",transition:"width 0.5s ease"}}/>
        </div>
      </div>
      {phase==="content"&&<div style={{padding:"20px 16px 0"}}>
        <div style={{maxWidth:480,margin:"0 auto"}}>
          <div style={{display:"inline-flex",alignItems:"center",padding:"5px 12px",borderRadius:20,marginBottom:10,background:"rgba(109,191,138,0.15)",border:"1px solid rgba(109,191,138,0.44)",fontSize:11,color:"#6dbf8a",fontWeight:700}}>{badgeText}</div>
