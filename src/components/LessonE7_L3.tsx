...
function buildBank(lang) {
  ...
  const qs=[
      ...
      {q:lbl("Que signifie 0 mA sur le signal governor ?","What does 0 mA on the governor signal mean?","Que significa 0 mA en la senal del governor?","O que significa 0 mA no sinal do governor?"),
        ...
        correct:0,expl:lbl("0 mA est impossible en fonctionnement normal (min = 4 mA). Indique une rupture de cable — failsafe : fuel rack en position securite.","0 mA is impossible in normal operation (min = 4 mA). Indicates cable break — failsafe: fuel rack in safe position.","0 mA es imposible en operacion normal (min = 4 mA). Indica ruptura de cable — failsafe.","0 mA e impossivel em operacao normal (min = 4 mA). Indica ruptura de cabo — failsafe.")},
      ...
      {q:lbl("Pourquoi le VDR enregistre-t-il les ordres telegraph ?","Why does the VDR record telegraph orders?","Por que el VDR registra las ordenes de telegrafo?","Por que o VDR registra as ordens de telegrafo?"),
        ...
        correct:1,expl:lbl("SOLAS V/20 : le VDR enregistre tous les ordres telegraph avec horodatage sur 48h minimum. En cas d'accident ou d'enquete, permet de reconstituer exactement la sequence des manoeuvres.","SOLAS V/20: VDR records all telegraph orders with timestamp for 48h minimum. In case of accident or investigation, allows exact reconstruction of maneuvering sequence.","SOLAS V/20: el VDR registra todas las ordenes de telegrafo con marca horaria durante 48h minimo.","SOLAS V/20: o VDR registra todas as ordens de telegrafo com carimbo de hora por 48h minimo.")},
    ];
  return qs;
}
...
export const QUIZ={
  fr:[
    {q:"Le governor electronique ajuste le fuel rack via :",opts:["Signal 4-20 mA","Cle mecanique","Signal 0-10V","Commande hydraulique directe"],correct:0,expl:"Signal standard 4-20mA : 4mA=0% rack, 20mA=100% rack, 0mA=rupture cable (failsafe). Insensible aux parasites electriques."},
    {q:"Pourquoi prechauffer le HFO a 130-140C avant injection ?",opts:["Augmenter la puissance","Reduire la viscosite de 380 a 12-15 cSt pour bonne atomisation","Respecter MARPOL","Refroidir les injecteurs"],correct:1,expl:"HFO 380 cSt est tres visqueux a temperature ambiante. Le chauffage abaisse la viscosite a 12-15 cSt pour une bonne pulverisation dans les cylindres et une combustion complete."},
    {q:"Quel ordre telegraph signifie la vitesse avant minimale ?",opts:["SLOW AHEAD","HALF AHEAD","DEAD SLOW AHEAD","STOP"],correct:2,expl:"Dead Slow Ahead = vitesse avant minimale, environ 25-30% MCR. Utilisé en maneuvre portuaire, canal ou zone encombrée. Doit etre exécuté immediatement apres ACK."},
    {q:"Un safety shutdown se declenche — que se passe-t-il si la passerelle demande FULL AHEAD ?",opts:["Le moteur repart automatiquement","Le shutdown est prioritaire — le moteur reste arrete","L'officier mecanicien choisit","Le capitaine override"],correct:1,expl:"Le safety shutdown a toujours priorite absolue sur tout ordre telegraph, y compris FULL AHEAD. L'equipage doit identifier et corriger la cause avant redemarrage."},
    {q:"Le MV Derbyshire (1980) a sombré avec le ME en marche car :",opts:["Le governor etait defaillant","Aucun systeme automatique ne detectait l'envahissement structurel","Le telegraph etait bloque","Le carburant etait epuise"],correct:1,expl:"Le governor et le ME fonctionnaient normalement. L'absence de detection d'inondation des espaces structurels a permis le naufrage sans alarme. Aujourd'hui : SOLAS exige detection inondation + VDR obligatoire."},
  ],
  en:[
    {q:"The electronic governor adjusts the fuel rack via:",opts:["4-20 mA signal","Mechanical key","0-10V signal","Direct hydraulic command"],correct:0,expl:"Standard 4-20mA signal: 4mA=0% rack, 20mA=100% rack, 0mA=cable break (failsafe). Immune to electrical interference."},
    {q:"Why preheat HFO to 130-140C before injection?",opts:["Increase power","Reduce viscosity from 380 to 12-15 cSt for good atomization","Comply with MARPOL","Cool injectors"],correct:1,expl:"HFO 380 cSt is very viscous at ambient temperature. Heating lowers viscosity to 12-15 cSt for good atomization in cylinders and complete combustion."},
    {q:"Which telegraph order means minimum ahead speed?",opts:["SLOW AHEAD","HALF AHEAD","DEAD SLOW AHEAD","STOP"],correct:2,expl:"Dead Slow Ahead = minimum ahead speed, about 25-30% MCR. Used in port maneuvering, canal or congested area. Must be executed immediately after ACK."},
    {q:"A safety shutdown triggers — what happens if bridge requests FULL AHEAD?",opts:["Engine restarts automatically","Shutdown has priority — engine remains stopped","Duty engineer chooses","Captain overrides"],correct:1,expl:"Safety shutdown always has absolute priority over any telegraph order, including FULL AHEAD. Crew must identify and correct the cause before restart."},
    {q:"MV Derbyshire (1980) sank with ME running because:",opts:["Governor was faulty","No automatic system detected structural flooding","Telegraph was stuck","Fuel was exhausted"],correct:1,expl:"Governor and ME were functioning normally. The absence of structural space flooding detection allowed sinking without alarm. Today: SOLAS requires flooding detection + mandatory VDR."},
  ],
  es:[
    {q:"El governor electronico ajusta la cremallera via:",opts:["Senal 4-20 mA","Llave mecanica","Senal 0-10V","Mando hidraulico directo"],correct:0,expl:"Senal estandar 4-20mA: 4mA=0% cremallera, 20mA=100%, 0mA=ruptura cable (failsafe). Inmune a interferencias electricas."},
    {q:"?Por que precalentar el HFO a 130-140C antes de la inyeccion?",opts:["Aumentar la potencia","Reducir la viscosidad de 380 a 12-15 cSt para buena atomizacion","Cumplir MARPOL","Enfriar los inyectores"],correct:1,expl:"HFO 380 cSt es muy viscoso a temperatura ambiente. El calentamiento reduce la viscosidad a 12-15 cSt para una buena pulverizacion en los cilindros."},
    {q:"?Que orden de telegrafo significa la velocidad minima de avance?",opts:["SLOW AHEAD","HALF AHEAD","DEAD SLOW AHEAD","STOP"],correct:2,expl:"Dead Slow Ahead = velocidad minima de avance, aprox. 25-30% MCR. Usado en maniobra portuaria, canal o zona congestionada."},
    {q:"Se activa un safety shutdown — ?que pasa si el puente pide FULL AHEAD?",opts:["El motor arranca automaticamente","El shutdown tiene prioridad — el motor permanece parado","El oficial de maquinas elige","El capitan tiene prioridad"],correct:1,expl:"El safety shutdown siempre tiene prioridad absoluta sobre cualquier orden de telegrafo, incluido FULL AHEAD."},
    {q:"El MV Derbyshire (1980) se hundio con el ME en marcha porque:",opts:["El governor estaba averiado","Ningun sistema automatico detectaba la inundacion estructural","El telegrafo estaba bloqueado","El combustible se agoto"],correct:1,expl:"El governor y el ME funcionaban normalmente. La ausencia de deteccion de inundacion de espacios estructurales permitio el hundimiento sin alarma."},
  ],
  pt:[
    {q:"O governor eletronico ajusta a cremalheira via:",opts:["Sinal 4-20 mA","Chave mecanica","Sinal 0-10V","Comando hidraulico direto"],correct:0,expl:"Sinal padrao 4-20mA: 4mA=0% cremalheira, 20mA=100%, 0mA=ruptura cabo (failsafe). Imune a interferencias eletricas."},
    {q:"Por que pre-aquecer o HFO a 130-140C antes da injecao?",opts:["Aumentar a potencia","Reduzir a viscosidade de 380 para 12-15 cSt para boa atomizacao","Cumprir MARPOL","Resfriar os injetores"],correct:1,expl:"HFO 380 cSt e muito viscoso na temperatura ambiente. O aquecimento reduz a viscosidade para 12-15 cSt para boa atomizacao nos cilindros."},
    {q:"Qual ordem de telegrafo significa a velocidade minima de avanco?",opts:["SLOW AHEAD","HALF AHEAD","DEAD SLOW AHEAD","STOP"],correct:2,expl:"Dead Slow Ahead = velocidade minima de avanco, aprox. 25-30% MCR. Usado em manobras portuarias, canal ou zona congestionada."},
    {q:"Um safety shutdown aciona — o que acontece se a ponte pede FULL AHEAD?",opts:["O motor reinicia automaticamente","Shutdown tem prioridade — motor permanece parado","Oficial de maquinas decide","Capitao tem prioridade"],correct:1,expl:"O safety shutdown sempre tem prioridade absoluta sobre qualquer ordem de telegrafo, incluindo FULL AHEAD."},
    {q:"O MV Derbyshire (1980) afundou com o ME a funcionar porque:",opts:["O governor estava avariado","Nenhum sistema automatico detetava a inundacao estrutural","O telegrafo estava bloqueado","O combustivel esgotou"],correct:1,expl:"O governor e o ME funcionavam normalmente. A ausencia de detecao de inundacao de espacos estruturais permitiu o afundamento sem alarme."},
  ],
};
